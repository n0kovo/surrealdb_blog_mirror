...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/spectron/self-hosting/deployment/kubernetes.mdx"
---
...
If SurrealDB fails with *Failed to create RocksDB directory* or a read-only filesystem error, check that **`workingDir` matches `mountPath`** and that the path slashes are correct — see [Docker storage paths](docker.md#storage-paths).
...
Update the image tag in your `Deployment` manifest and apply it. The rolling update strategy ensures zero-downtime upgrades. Pending per-Context schema migrations are applied asynchronously by the scheduler and workers — see [Migrations and upgrades](../operations/migrations-and-upgrades.md).
...
- [Storage and scaling](storage-and-scaling.md) – object store configuration and scaling advice
- [Authentication](../security/authentication.md) – creating and managing API keys
- [Observability](../observability/tracing.md) – monitoring your deployment
