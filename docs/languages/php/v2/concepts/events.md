...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/php/v2/concepts/events.mdx"
---
...
Use [`subscribe()`](../api/core.md#subscribe) for quick hooks into the connection lifecycle. Pass an event name and a listener; the method returns a closure that removes the listener when called.
...
> [!NOTE]
> `RpcRequestSent` and `RpcResponseReceived` fire for every call, so keep their listeners cheap. For tracing and metrics, prefer the dedicated [telemetry](observability.md) seam, which is built on this same pipeline.
...
- [Surreal API reference](../api/core.md#subscribe) for the `subscribe()` signature
- [Observability](observability.md) for tracing and metrics
- [Connecting to SurrealDB](connecting-to-surrealdb.md) for the lifecycle these events track
