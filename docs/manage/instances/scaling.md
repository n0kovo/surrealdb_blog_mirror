...
The steps for making a change are in [Configure an instance](configure.md#compute-and-storage).
...
Read those signals from the [instance dashboard, metrics, and logs](monitoring.md).
...
Moving from Start to Scale is not an in-place upgrade. Deploy a Scale instance, then [restore a backup](backups.md) or [import an export](import-and-export.md) into it.
...
See [Architecture](architecture.md) for the topologies, and [High availability](high-availability.md) for what each one survives.
...
`surrealctl instance metrics` and `surrealctl instance update` let you read the signals and apply the change from a terminal. See [surrealctl instances](../surrealctl/instances.md).
