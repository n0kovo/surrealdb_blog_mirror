...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/php/v2/concepts/error-handling.mdx"
---
...
The SDK checks the server version on connect. If it is outside the supported range, it throws `UnsupportedVersionException` with the reported version and the supported bounds. Disable the check with the `versionCheck` option on [`connect()`](connecting-to-surrealdb.md#connection-options).
...
- [Surreal API reference](../api/core.md) for the methods that raise these errors
- [Connecting to SurrealDB](connecting-to-surrealdb.md) for version and reconnection settings
- [Authentication](authentication.md) for handling sign-in failures
