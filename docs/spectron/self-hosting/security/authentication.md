...
For local single-process bring-up, **`spectrond dev start --bootstrap`** seeds the same keys on an empty database then serves (idempotent on restart). Pin a fixed Context key with **`--context-api-key`** / **`SPECTRON_API_KEY`** so clean databases do not mint a new secret each time — see [CLI — bootstrap](../../reference/cli.md#bootstrap).

Additional Context keys are minted via:

- Management REST: `POST /api/v1/contexts/{id}/keys/{name}`
- CLI: `spectrond keys generate-key` ([CLI reference](../../reference/cli.md))
...
MCP clients (Claude Desktop, Cursor, Claude Code) send the same Bearer token in the MCP transport config (`Authorization: Bearer …`). Point the client at your instance's `/mcp` endpoint — see the [MCP server install guide](../../integrations/mcp-server/install.md).
...
Alternatively, mint a replacement key and delete the old one — see [API keys and delegation](api-keys-and-delegation.md#key-rotation).
