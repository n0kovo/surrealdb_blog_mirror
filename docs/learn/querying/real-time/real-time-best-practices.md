...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/querying/real-time/real-time-best-practices.mdx"
---
...
To make the output nice, we'll use a little bit of SurrealQL magic. First we'll put the two queries creating the records inside their own scope, and use the [value::chain()](../../../reference/query-language/functions/database-functions/value.md#chain) function to grab the output, ignore it, and turn it into a string showing what operation has just been performed. Then we'll do the same for a regular `SELECT` query over the `weather2` data, compared to the record range query for the `weather` data. If you need the query to stop when a step does not look right, use [`.expect()`](../../../reference/query-language/functions/database-functions/value.md#valueexpect) on that value instead of `.chain()` to return the original value when your closure returns `true`, otherwise failing with an error.
...
To remove the precision from the original datetime, you can use the [`time::floor()`](../../../reference/query-language/functions/database-functions/time.md#timefloor) function.
...
If you don't want to make a query every time you want to see the latest updates to a table, you can use a [live query](../../../reference/query-language/statements/live-select.md) instead.
...
Here is one example of an event that not only creates an `alert` whenever a weather condition is set to critical, but can even use an [http function](../../../reference/query-language/functions/database-functions/http.md) to let an external service know about it.
...
Note that you will need to use the [--allow-net](../../security/authorization/capabilities.md#network) flag when starting up a SurrealDB instance to allow functions like `http::post()` to work, as they are disabled by default.
...
For a full list of aggregate functions, see [this page](../../../reference/query-language/functions/database-functions/index.md#aggregate-functions).
