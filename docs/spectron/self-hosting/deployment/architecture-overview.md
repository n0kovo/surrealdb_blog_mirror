...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/spectron/self-hosting/deployment/architecture-overview.mdx"
---
...
For a single-node SurrealDB deployment in Docker or Kubernetes, set **`working_dir` / `workingDir`** to match the volume mount and use **`SURREAL_PATH=rocksdb://…`** (relative to that directory) — see [Docker deployment](docker.md#storage-paths). Spectron itself only needs a **`ws://` or `wss://`** endpoint; it does not open the RocksDB path directly. See also [SurrealDB file-backed storage](../../../running/file-backed.md).
...
See [Storage and scaling](storage-and-scaling.md).
...
See [Hosted quickstart](https://surrealdb.com/docs/spectron/quickstarts/hosted) for a minimal bring-up sequence on SurrealDB Cloud.
