...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/explore/tutorials/tutorials/http-via-postman.mdx"
---
...
Check out the [HTTP & REST integration documentation](../../../reference/rest-api/http-protocol.md) for a detailed list of all the endpoints supported.
...
After running locally, head over to Postman and create a fork for your workspace. You should see all the endpoints listed. You can also [check the endpoints in the documentation](../../../reference/rest-api/http-protocol.md). 

## Using the `INFO` statement

You can get information about your workspace by using the INFO statement. However, you need the right permissions in order to see the output. See the documentation for the [INFO statement](../../../reference/query-language/statements/info.md).

Note that the `namespace` and `database` values for the Postman collection are set to `test` by default. 

You can see the JSON output using the [INFO statement](../../../reference/query-language/statements/info.md) in the body of the request.
...
In the request body, [define the record access method](../../../reference/query-language/statements/define/access/record.md) `human` with a session of 24 hours: 
...
The code above allows a user to sign up as a record user with their email and password, and hashes the password with the [`crypto::argon2::generate`](../../../reference/query-language/functions/database-functions/crypto.md#cryptoargon2generate) function.

The sign in logic gets all the users where the emails match and uses the [`crypto::argon2::compare`](../../../reference/query-language/functions/database-functions/crypto.md#cryptoargon2compare) function to compare the stored hash with the password provided.
...
The Postman collection has a couple of endpoints for `POST` `GET` `PUT` `PATCH` `DEL` operations. For example, if you want to add a new entry to the table `Person` (which is the default table in the collection), go to the [`POST /key/:table`](../../../reference/rest-api/http-protocol.md#post-table) endpoint and add the table content in the body of the request. For example:
...
You should get a response as seen below, notice that there is a [record ID](../../../reference/query-language/language-primitives/data-types/record-ids.md), with this record ID you can now use any of the [`/key/:table/:id`](../../../reference/rest-api/http-protocol.md#get-record) endpoints for  `POST` `GET` `PUT` `PATCH` `DEL` operations. 
...
The Postman collection is still in active development. For more information on using SurrealDB via the HTTP endpoints, [check out our documentation on HTTP and REST endpoints](../../../reference/rest-api/http-protocol.md).
