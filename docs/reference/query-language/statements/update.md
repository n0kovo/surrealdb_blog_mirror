...
```
                     ...
                     │            │                  │ │                                                  │ │ │                                   │ │                 │                                         │ │ │                                    │ │                  │            │ │               
        ╭────────╮   │  ╭──────╮  │   ┌──────────┐   │ │             ╭───────╮     ┌────────┐             │ │ │    ╭───────╮     ┌────────────┐   │ │    ╭────────╮   │               ╭────────╮                │ │ │    ╭─────────╮     ┌───────────┐   │ │    ╭─────────╮   │  ╭──────╮  │ │   ╭───╮       
├┼──────│ UPDATE │───╯──│ ONLY │──╰───│ @targets │───╯─┼─────────────│ MERGE │─────│ @value │─────────────┼─╰─╯────│ WHERE │─────│ @condition │───╰─╯────│ RETURN │───┼───────────────│ BEFORE │────────────────┼─╰─╯────│ TIMEOUT │─────│ @duration │───╰─╯────│ EXPLAIN │───╯──│ FULL │──╰─╰───│ ; │─────┼┤
        ╰────────╯      ╰──────╯      └──────────┘     │             ╰───────╯     └────────┘             │        ╰───────╯     └────────────┘          ╰────────╯   │               ╰────────╯                │        ╰─────────╯     └───────────┘          ╰─────────╯      ╰──────╯        ╰───╯       
                                                       │                                                  │                                                           │                                         │                                                                                            
                                                       │             ╭───────╮     ┌────────┐             │                                                           │                ╭───────╮                │                                                                                            
                                                       ╰─────────────│ PATCH │─────│ @value │─────────────╯                                                           ╰────────────────│ AFTER │────────────────╯                                                                                            
...
> [!NOTE]
> `@target` refers to either record output including an `id` field, or a [record ID](../language-primitives/data-types/record-ids.md) on its own.

## Example usage

The examples below show how to use the `UPDATE` statement. They start by creating two `person` records with the [`CREATE`](create.md) statement so that the examples produce meaningful output.
...
The `WHERE` condition is checked before the data clause is evaluated, so a data clause with side effects only runs for the records the condition matches. The same applies to [`UPSERT`](upsert.md#evaluation-order) when it updates an existing record.
...
This has always held for ordinary fields - `SET a = a + 1, b = a + 1` assigns `b` from the old `a`. [Computed fields](define/field.md#restrictions-on-computed-fields) are now part of that image too, so a data clause and a condition both see their pre-mutation values.
...
A statement that creates a record has no earlier image, so a computed field reads `NONE` there - whether the record ID was named, generated, or reached through [`UPSERT`](upsert.md).
...
Writing to a [set](../language-primitives/data-types/sets.md) by position reaches whichever element sorts into that place, so a set is rebuilt once the patch has been applied.
...
Every statement that takes a `PATCH` clause behaves this way, so the same applies to [`CREATE`](create.md), [`UPSERT`](upsert.md), [`RELATE`](relate.md) and [`INSERT`](insert.md).
...
As `UPDATE` before version 2.0.0 used to create a specified record ID if it did not exist, it was used in the `.surql` files generated by the [`surreal export`](../../cli/surrealdb-cli/commands/export.md) command to export existing records in a database. As of version 2.0.0, the [`INSERT`](insert.md) statement is used instead.
