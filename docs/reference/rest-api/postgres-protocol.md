...
Every new connection uses the **SurrealQL** dialect unless configured otherwise. Send SurrealQL as you would on [`POST /sql`](http-protocol.md):
...
- [`surreal start`](../cli/surrealdb-cli/commands/start.md): CLI reference including `--postgres-bind`
