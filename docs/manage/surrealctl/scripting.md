...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/manage/surrealctl/scripting.mdx"
---
...
Put a personal access token in the environment and leave it out of `argv`, where `ps` and shell history can both read it. See [Authentication](authentication.md#personal-access-tokens) for how to create one.
...
> [!IMPORTANT]
> A personal access token reads the control plane and cannot write to it. A job that creates, updates, or deletes needs a login session on a dedicated machine account. See [What a personal access token cannot do](authentication.md#what-a-personal-access-token-cannot-do).
...
- [Authentication](authentication.md) — credentials, scopes, and profiles.
- [Instances](instances.md) — the workflows these scripts drive.
- [`surrealctl` reference](../../reference/cli/surrealctl/overview.md) — every command, flag, default, and environment variable.
