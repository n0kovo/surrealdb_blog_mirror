...
This command takes no positional argument, deliberately: a positional here would read as an *instance*. The organisation comes from `--org` and the [precedence chain](../global-flags.md#the-precedence-chain).

Accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags) and nothing else. Column ids are `name`, `state`, `type`, `version`, `region`, `compute_units` and `storage`, with `id`, `slug`, `host`, `access_type` and `organization_id` under `--wide`.
...
A type change also resets the storage cool-off, so passing `--type` and `--storage` together fails at the storage step, in which the type is changed and the storage request is refused. The [Storage limits on Start](../../../../manage/instances/scaling.md#storage-limits-on-start) page lists the storage each type starts with.
...
Also accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags), which arrange the snapshot table. Column ids are `snapshot_id`, `started_at`, `tiers` and `on_demand`, with `valid_versions` under `--wide`.
...
This command does not speak SurrealQL. It resolves the instance, mints a database token, and becomes [`surreal sql`](../../surrealdb-cli/commands/sql.md). Everything after `--` is forwarded to that command verbatim, because the sibling's flag surface is large and moves independently.
...
`--namespace` and `--database` are optional here, because `surreal sql` accepts a session without them. See [the `surreal` handoff](../../../../manage/surrealctl/install.md#the-surreal-handoff) for how the binary is located and what is passed in the child's environment.
...
See [`surreal import`](../../surrealdb-cli/commands/import.md) for the flags available after the `--`.
...
Like `import`, this refuses a missing namespace or database before minting anything. See [`surreal export`](../../surrealdb-cli/commands/export.md) for the flags available after the `--`.
...
Also accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags). Summary rows carry `series`, `last`, `min`, `mean`, `max` and `unit`, with `samples` and `gaps` under `--wide`; sample rows carry `timestamp`, `series` and `value`, with `unit` under `--wide`.
...
Also accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags). Column ids are `timestamp`, `level` and `message`, with `pod` under `--wide`.
...
For every instance in the organisation at once, use [`org usage`](org.md#org-usage).
...
Accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags) and nothing else. Column ids are `snapshot_id`, `started_at`, `tiers` and `on_demand`, with `valid_versions` under `--wide`.
