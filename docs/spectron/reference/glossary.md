...
The precedence rule: **Authoritative** (curated) content takes precedence over **Experiential** assertions when they conflict. Disagreements are surfaced via **`uncertainty`** records and conflict indicators in state/profile responses — curated records are not silently overwritten. See [Authority when pillars meet](../memory/reasoning-model/authority-hierarchy.md).
...
A set of key-value tags (e.g. `{org: "acme", user: "alice"}`) that partitions data within a Context. Every piece of data in Spectron carries a scope, and queries filter by scope using subset matching semantics. See [Contexts and scope](https://surrealdb.com/docs/spectron/mental-model/contexts-and-scope).
