...
### IDs inside IDs
The last thing to note here is that we’ve actually been using record IDs inside our complex record IDs! This is known as a [record link](../../../reference/query-language/language-primitives/record-links.md), which allows us to reduce the fields in our main time series table to only the necessary ones by offloading most of the metadata to connected tables.
