...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/agent-memory/reference/mcp-tools.mdx"
---
...
> [!NOTE]
> REST responses use **camelCase** (`queryMs`, `traceId`, `trace.traceId`). **Scope selectors** on the wire use DNF (`ScopeSets`): writes take **`scope`** / **`scopes`**, reads take **`lens`** - an OR of conjunctive slash-path clauses. Register paths with `agent-memory scopes create` before use. See [Contexts and scope](https://surrealdb.com/docs/agent-memory/mental-model/contexts-and-scope#wire-format-scopesets).
