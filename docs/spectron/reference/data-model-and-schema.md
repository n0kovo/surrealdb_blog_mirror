...
Structured authoritative facts extracted from documents (or ingested via `POST /facts` with `infer: "triples"`) are stored as **`entity`**, **`attribute`**, and **`relates_to`** records with `source.kind = "document"`. Connectors are not yet implemented; see [Connectors overview](../knowledge/connectors/overview.md).
