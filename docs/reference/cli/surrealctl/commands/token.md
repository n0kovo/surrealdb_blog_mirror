...
Being signed out is a different answer with a different code: `3`, not `4`. Sign in with [`auth login`](auth.md#auth-login).

These tokens are not database credentials. For a token a SurrealDB client can authenticate with, use [`instance token`](instance.md#instance-token).
...
Accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags) and nothing else. Column ids are `label`, `id`, `scopes`, `created_at` and `expires_at`, with `user_id` under `--wide`.
...
Accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags) and nothing else. Column ids are `id`, `category` and `label`, with `description` under `--wide`.
...
This lists what a token *can* be granted. [`auth scopes`](auth.md#auth-scopes) lists what the credential in hand actually carries.
