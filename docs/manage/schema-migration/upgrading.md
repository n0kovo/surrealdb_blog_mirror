...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/manage/schema-migration/upgrading.mdx"
---
...
The single-file seed fallback was deprecated in 0.6, where it printed a warning on every run, and is now removed. Move it into the [seed directory](seeding.md):
...
[Schema modules](modules-and-targets.md) leave your existing schema in the default module with its metadata untouched, and new modules live alongside it:
...
[Targets](modules-and-targets.md) name the databases to apply modules to, taking their passwords from the environment:
...
Both live in [`surrealkit.toml`](configuration.md), which is now discovered by walking up from the working directory rather than being read from that directory alone.

## Next steps

- [Configuration](configuration.md): the full `surrealkit.toml` key reference
- [Schema modules and targets](modules-and-targets.md): several schemas across several databases
- [Seeding](seeding.md): the seed directory and the tracking table
