...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/php/v2/concepts/runtimes.mdx"
---

# Runtimes and workers

By default the SDK runs on the standard synchronous PHP runtime. That suits request and response work: connect, run a query, return a result. Long-lived operations such as [live queries](live-queries.md) block the process that runs them until they end, so they need a runtime and worker setup that keeps the rest of your application responsive.
...
See [Observability](observability.md#runtime-presets) for the full setup and options.
...
> [!NOTE]
> For Laravel applications, [Laravel Octane](https://laravel.com/docs/octane) runs your app on Swoole, FrankenPHP, or RoadRunner with a configurable `--workers` count. See the [Laravel integration](../../frameworks/laravel/index.md).

## Learn more

- [Live queries](live-queries.md) for the subscription API
- [Connecting to SurrealDB](connecting-to-surrealdb.md) for connection setup
- [Utilities](../api/utilities.md#driveroptions) for the `DriverOptions` the presets configure
