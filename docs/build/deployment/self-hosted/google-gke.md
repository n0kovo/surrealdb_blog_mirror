...
> [!NOTE]
> This guide deploys a **highly available cluster** on GKE and connects SurrealDB to it. For a **single-node RocksDB** deployment on Kubernetes, start with [Deploy on Kubernetes](kubernetes.md). For managed deployments, see [SurrealDB Cloud](../surrealdb-cloud/what-is-surrealdb-cloud.md).
