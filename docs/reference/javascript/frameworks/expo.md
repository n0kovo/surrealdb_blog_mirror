...
The SDK surface is the same one used in [React](react.md) applications, but three constraints apply on Android and iOS that do not apply in a browser.

- **Embedded engines are not supported.** The [WebAssembly engine](../engines/wasm.md) needs a WebAssembly runtime, which Hermes does not provide. The [Node.js engine](../engines/node.md) is a native Node addon and cannot be loaded by React Native. Every connection from an Expo app is a remote connection.
...
> [!NOTE]
> The SDK needs `TextEncoder`, `TextDecoder`, `URL`, and `URLSearchParams` at runtime. The `expo` package installs all of them as globals on Android and iOS, so no polyfills are required. A bare React Native project has to add them itself — see the [React Native guide](react-native.md#installing-the-required-polyfills).
...
Use `npx expo install` rather than your package manager directly. It picks dependency versions that match your Expo SDK. Follow the [installation guide](../installation.md) for more information on how to install the SDK in your project.
...
Use the `useSurrealClient()` hook to reach the `Surreal` instance from any component. All [query methods](../concepts/executing-queries.md) are available on the client, including `.query()`, `.select()`, and `.create()`.
...
- [React Native](react-native.md) for the same setup without Expo, including the polyfills you must add yourself
