...
The clause takes record ids only, and a record the transaction writes needs no clause, because the write is conflict-checked in its own right. See [Transactions and isolation](../../../transactions-and-isolation.md#locked-reads-with-for-update) for the full behaviour and limits.
