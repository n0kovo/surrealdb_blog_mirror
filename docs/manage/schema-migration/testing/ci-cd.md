...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/manage/schema-migration/testing/ci-cd.mdx"
---
...
A project using [`surrealkit.toml`](../configuration.md) needs that file inside the container too. The image sets no working directory, so mount it at `/home/nonroot/surrealkit.toml` or give the service a `working_dir`.
...
Seed files are tracked, so a job that reuses a database applies only what changed. Pass `--force` to `surrealkit seed` when a job needs every seed file re-applied. See [Seeding](../seeding.md).
