...
SurrealDB is source-available (see the code [here on GitHub](https://github.com/surrealdb/surrealdb)) and is also available as a cloud managed service through [SurrealDB Cloud](manage/cloud/index.md).
...
- **Native ACID compliance:** [snapshot isolation](transactions-and-isolation.md) on every transaction, with immediate consistency after a commit. Opt in to eventual consistency in certain cases if desired.
