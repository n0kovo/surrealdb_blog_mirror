...
The check follows calls to [custom functions](function.md), so a write reached through a function call is caught in the same way as one written directly into the clause, and the error names the function. It also applies to [`ALTER`](../alter/overview.md) as of 3.3.0 — `ALTER TABLE`, `ALTER FIELD`, `ALTER PARAM`, `ALTER MODULE`, and `ALTER BUCKET` refuse a clause that the matching `DEFINE` would refuse.
