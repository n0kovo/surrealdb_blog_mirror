...
**Requires server configuration:** set `SPECTRON_RERANKER_URL` and `SPECTRON_RERANKER_MODEL` at startup (see [Configuration](../../reference/configuration.md#reranker-optional)). Without a reranker provider, `use_reranker=true` falls through to bi-encoder ordering.
