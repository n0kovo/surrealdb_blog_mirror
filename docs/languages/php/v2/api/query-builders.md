...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/php/v2/api/query-builders.mdx"
---
...
| `compile()` | [`BoundQuery`](utilities.md#boundquery) | Compile to SurrealQL and bindings without running |
...
Execute a pre-built [`BoundQuery`](utilities.md#boundquery). Returns one result per statement.
...
`where()` accepts a SurrealQL string or a [`BoundQuery`](utilities.md#boundquery) fragment. Use a `BoundQuery` to keep dynamic values parameterised.
...
- [Utilities](utilities.md) for `BoundQuery` and the `Output` enum
