...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/cli/surrealctl/long-running-operations.mdx"
---
...
Five commands carry all three flags: [`instance create`](commands/instance.md#instance-create), [`instance update`](commands/instance.md#instance-update), [`instance delete`](commands/instance.md#instance-delete), [`instance pause`](commands/instance.md#instance-pause) and [`instance resume`](commands/instance.md#instance-resume).
...
Two commands that might look asynchronous are not: [`instance backup create`](commands/instance.md#instance-backup-create) carries no wait flags, and [`instance watch`](commands/instance.md#instance-watch) carries `--wait-timeout` only, because a watch is nothing but a wait.
...
In plain and CI output, one line is printed per state change plus a heartbeat every 60 seconds, so a log stays readable and a stalled wait is still visibly alive. Under `--json`, transitions are [newline-delimited JSON on stderr](output-and-exit-codes.md#ndjson-streams) and the final document goes to stdout.
...
Interrupting a wait does not stop the operation — the CLI is only polling, and the change carries on server-side. Pick the state back up with [`instance get`](commands/instance.md#instance-get) for a snapshot, or [`instance watch`](commands/instance.md#instance-watch) to resume waiting.
...
> [!WARNING]
> `instance create` has no idempotency key, because the API's create route accepts none. A retried create is a second instance and a second bill. If a create times out, run [`instance list`](commands/instance.md#instance-list) before running it again.

## Related pages

- [`instance` commands](commands/instance.md) — every command that waits
- [Output and exit codes](output-and-exit-codes.md) — the NDJSON progress stream and the full exit-code table
- [Global flags](global-flags.md) — `--timeout` and `--retries`, which govern a single request rather than a wait
- [Overview](overview.md) — the rest of the reference
- [SurrealDB CLI](../surrealdb-cli/overview.md) — for connecting to the instance once it is ready
