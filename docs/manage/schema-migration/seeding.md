...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/manage/schema-migration/seeding.mdx"
---
...
The hash is taken from the file on disk, before [template variables](template-variables.md) are substituted. Changing a `--var` value therefore does not re-run a seed on its own. Edit the file, or use `--force`.
...
Like the other SurrealKit metadata tables, `__seed` is managed by the tool and should not be edited directly, except when [upgrading from an older layout](upgrading.md).
...
An application that ships without its `database/` directory can bake the seed files into the binary at compile time with the `embed_seed!` macro, the counterpart to [`embed_schema!`](embed-schema-macro.md):
...
> [!NOTE]
> `surrealkit seed` reads `database/seed` regardless of `--schema`, so a project using [schema modules](modules-and-targets.md) cannot yet seed a named module's directory from the CLI in `1.0.0-beta.1`. Use the library's `Seed` builder, or point `--folder` at the module.

## Next steps

- [Sync](sync.md): apply schema definitions to a database
- [Template variables](template-variables.md): parameterise files per environment
- [Using SurrealKit as a library](library/index.md): drive seeding from Rust
