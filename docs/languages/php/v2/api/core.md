...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/php/v2/api/core.mdx"
---
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td>`string | Endpoint`</td>
            <td>The endpoint to connect to, such as `ws://127.0.0.1:8000/rpc`.</td>
        </tr>
        <tr>
            <td>`options` <label label="optional" /></td>
            <td>`<a href="#connectoptions">ConnectOptions</a>`</td>
...
Sign in with a [credential object](../concepts/authentication.md#credential-types) or an array. Returns a [`Tokens`](#tokens) object.
...
Execute a [`BoundQuery`](utilities.md#boundquery), returning one result per statement.
...
These methods start a fluent [query builder](query-builders.md). Call `execute()` to run it.
...
Iterating the result blocks the process while it waits for messages. See [running without blocking the application](../concepts/live-queries.md#running-without-blocking-the-application) for consuming live queries in a worker.
...
- [Data types](data-types.md) for the value classes
- [Utilities](utilities.md) for `BoundQuery`, enums, and driver options
- [Connecting to SurrealDB](../concepts/connecting-to-surrealdb.md) for the connection guide
