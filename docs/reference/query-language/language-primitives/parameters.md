...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/query-language/language-primitives/parameters.mdx"
---
...
However, the parameter `$USERNAME` in this case is still defined as its original value, as can be seen via an [`INFO FOR DB`](../statements/info.md) statement.
...
These three parameters are used in the context of the permissions of a [`DEFINE BUCKET`](../statements/define/bucket.md) statement.
...
This parameter represents the value of a request to a custom API defined using the [`DEFINE API`](../statements/define/api.md) statement.
