...
`eval::gql` requires the **`gql` experimental capability** (same as [`POST /gql`](../../../rest-api/http-protocol.md#gql)) and runs on the streaming query engine. Queries that include GQL **mutations** (`INSERT`, `SET`, `REMOVE`, `DELETE`) participate in the caller's write transaction — see [GQL mutations](../../../../learn/querying/gql/mutations.md).
