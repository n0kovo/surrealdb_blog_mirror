...
```
                                   ...
                                   │ │                                        │ │                          │                               │                                                         │                                  │     
        ╭────────╮     ╭───────╮   │ │    ╭────╮     ╭─────╮     ╭────────╮   │ │   ┌───────┐     ╭────╮   │         ╭──────────╮          │   ╭──────╮     ┌───────┐     ╭───────╮     ┌────────┐   │    ╭─────────╮     ┌─────────┐   │     
├┼──────│ DEFINE │─────│ TOKEN │───╯─╯────│ IF │─────│ NOT │─────│ EXISTS │───╰─╰───│ @name │─────│ ON │───┼─────────│ DATABASE │──────────┼───│ TYPE │─────│ @type │─────│ VALUE │─────│ @value │───╯────│ COMMENT │─────│ @string │───╰───┼┤
        ╰────────╯     ╰───────╯          ╰────╯     ╰─────╯     ╰────────╯         └───────┘     ╰────╯   │         ╰──────────╯          │   ╰──────╯     └───────┘     ╰───────╯     └────────┘        ╰─────────╯     └─────────┘         
                                                                                                           │                               │                                                                                                  
                                                                                                           │    ╭───────╮     ┌────────┐   │                                                                                                  
                                                                                                           ╰────│ SCOPE │─────│ @scope │───╯                                                                                                  
...
- [You must select your namespace and/or database](../use.md) before you can use the `DEFINE TOKEN` statement for database or namespace tokens.
...
For tokens defined `ON NAMESPACE` and `ON DATABASE`, the optional `rl` claim containing an array of capitalized [system user roles](user.md#roles) (e.g. `["Viewer", "Editor", "Owner"]`) can be provided. Doing so will apply the access policy for those roles to any action made using the token. By default, tokens without the `rl` claim will only have the `Viewer` role.
