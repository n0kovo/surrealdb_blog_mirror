...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/php/v2/concepts/sessions.mdx"
---
...
Additional sessions are managed through the [`ConnectionController`](../api/core.md#connectioncontroller), which you reach with `connection()`.
...
The same call accepts a transaction id as a third argument, so a session can run statements inside an explicit [transaction](transactions.md).

## Learn more

- [Surreal API reference](../api/core.md#connectioncontroller) for the session methods
- [Transactions](transactions.md) for running statements atomically
- [Connecting to SurrealDB](connecting-to-surrealdb.md) for the default session and WebSocket setup
