...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/spectron/cookbooks/patterns/historical-and-archaeological-data.mdx"
---
...
- **Leave similar names alone.** `Holconius`, `Marcus Holconius`, and `Marcus Holconius Priscus` may be one person or three. Let resolution and [reconciliation](../../agent-memory/reasoning/reconciliation-and-supersession.md) decide. A manual merge destroys the evidence for the decision.
...
Reads then accept `asOf`. You can ask what the corpus attests for one moment rather than for all time. See [Temporal validity](../../agent-memory/reasoning/temporal-validity.md) and [spoiler-safe narrative memory](spoiler-safe-narrative-memory.md) for more on this subject.
...
`k` on `/query` defaults to **10** and deployments cap it at **50**. The internal retrieval pool is set separately from `k`. See [Answer size vs search breadth](../../agent-memory/retrieve/hybrid-search.md#answer-size-vs-search-breadth).
...
- **Turn on query expansion for document retrieval.** `/documents/query` accepts `useHyde` and `decomposeQuery`. Both help when query wording and corpus wording diverge. See [Hybrid search](../../agent-memory/retrieve/hybrid-search.md#advanced-options).
...
3. **`/reflect`** for interpretive claims across many records. Reflection always calls a model. It returns synthesised text with its evidence. See [Reflection](../../agent-memory/operations/reflect.md).
...
- **Use a fresh Context per experiment.** Operator questions and prior answers become hits, so a reused Context contaminates the next run. See [Contexts and scope](https://surrealdb.com/docs/spectron/mental-model/contexts-and-scope).
