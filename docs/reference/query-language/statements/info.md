...
The information returned from an `INFO` command is an object containing items that almost always correspond to a matching [DEFINE](define/overview.md) statement. For example, the `INFO FOR NS` command returns the information on the access methods, databases and users of a namespace, which are defined with `DEFINE ACCESS`, `DEFINE DATABASE` and `DEFINE USER` statements.
...
The `USER` command returns information for a user [defined](define/user.md) on either the root, namespace, or database level.
...
The status is most useful when the [`CONCURRENTLY`](define/indexes.md#using-concurrently-clause) clause is used in a `DEFINE INDEX` command. Without this clause, the following statement will not be executed until the index is fully created, or fails, so `INFO FOR INDEX` reports an index that is already `ready`.
...
The output of an `INFO` statement, both with and without the `STRUCTURE` clause, can be used in other operations. As the output of the statement is always a single object, the SurrealQL [object functions](../functions/database-functions/object.md) can also be used on the output for such tasks as schema change tracking.
...
As `INFO` statements return an object and objects can be turned into arrays using SurrealDB's [object functions](../functions/database-functions/object.md), all of the statements for a schema can be constructed using `INFO` statements alone. This query for example shows each of the statements found in a single database.
...
A namespace and database with the name of `name` are generated by default when starting a SurrealDB instance unless the `SURREAL_NO_DEFAULTS` [environment variable](../../cli/surrealdb-cli/environment-variables.md) is set to false or the [`--no-defaults`](../../cli/surrealdb-cli/commands/start.md) flag is used.
