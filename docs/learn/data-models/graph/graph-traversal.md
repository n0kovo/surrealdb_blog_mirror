...
Square brackets look and act similar but are a different mechanism. `[WHERE ...]` is the generic [array filter](../../../reference/query-language/language-primitives/data-types/arrays.md#filtering-and-mapping-with-array-functions), so `->wrote[WHERE location = "Arizona"]->comment` walks every `wrote` edge and then filters the array that comes back. Inside the parentheses the condition is part of the path instead, and becomes the edge scan's own predicate, so edges that fail it are never followed. Both return the same records, and [`EXPLAIN`](../../../reference/query-language/statements/explain.md) shows the difference: the parenthesised form carries a `predicate` on the scan, the bracketed one carries none. On a record with a few edges the difference does not matter, while on one with thousands, only the first stays fast.
...
To drop the current record’s own id from the list, use [`array::complement()`](../../../reference/query-language/functions/database-functions/array.md#arraycomplement):
...
For more on this pattern, see the [`RELATE` statement](../../../reference/query-language/statements/relate.md#bidirectional-relation-querying) and [Chapter 7 of Aeon's Surreal Renaissance](/learn/book/chapter-07#bidirectional-querying-when-a-relationship-is-equal).
...
Paths are not limited to `SELECT`; they can appear in [`DEFINE FIELD`](../../../reference/query-language/statements/define/field.md):
...
* [Recursive traversals](recursive-traversals.md) for `@.{n}` and nested recursive shapes.
* [Idioms: recursive paths](../../../reference/query-language/language-primitives/idioms.md#recursive-paths)
