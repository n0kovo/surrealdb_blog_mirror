...
Licence state is reported only through the `surrealdb_license_*` gauges on `/metrics`, which an anonymous scrape does not return. See [Licence metrics](../observability/metrics.md#licence---surrealdblicense).

The Enterprise build is published on Docker Hub as [`surrealdb/surrealdb-enterprise`](https://hub.docker.com/r/surrealdb/surrealdb-enterprise). [Run with Docker](../self-hosted/docker.md#running-surrealdb-enterprise) has an example that passes the licence key.
