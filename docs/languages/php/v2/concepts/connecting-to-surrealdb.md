...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/php/v2/concepts/connecting-to-surrealdb.mdx"
---
...
<table>
    ...
    <tbody>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/languages/php/v2/api/core#connect"> ` $db->connect($url, $options) `</a></td>
            <td scope="row" data-label="Description">Connects to a local or remote database endpoint</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/languages/php/v2/api/core#close"> ` $db->close() `</a></td>
            <td scope="row" data-label="Description">Closes the connection to the database</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/languages/php/v2/api/core#use"> ` $db->use($namespace, $database) `</a></td>
            <td scope="row" data-label="Description">Switches to a specific namespace and database</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/languages/php/v2/api/core#status"> ` $db->status() `</a></td>
...
Create a `Surreal` instance, then call `connect()` with a connection string and a [`ConnectOptions`](../api/core.md#connectoptions) object.
...
Passing credentials to `connect()` is the preferred way to authenticate, because it lets the SDK re-authenticate automatically when a WebSocket connection drops and reconnects. You can also pass a token, or a closure that returns credentials. See [Authentication](authentication.md) for the credential types.
...
The `status()` method returns a [`ConnectionStatus`](../api/core.md#connectionstatus) enum with one of four values:
...
- [Surreal API reference](../api/core.md) for the full connection interface
- [Authentication](authentication.md) for signing in and managing credentials
- [Error handling](error-handling.md) for connection and version errors
