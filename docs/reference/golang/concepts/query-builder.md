...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/golang/concepts/query-builder.mdx"
---
...
The `surrealql` query builder lets you construct SurrealQL queries programmatically using a fluent Go API. It automatically binds values as parameters to prevent injection, and produces a query string and variables map that you can pass directly to [`surrealdb.Query`](../api/core/db.md#query).
...
Every query builder type has a `.Build()` method that returns a SurrealQL string and a `map[string]any` of parameters. Pass these directly to [`surrealdb.Query`](../api/core/db.md#query):
...
`SelectOnly` produces a `SELECT ... ONLY` query that returns a single record instead of an array. You can pass [`RecordID`](../api/values/record-id.md) and [`Table`](../api/values/table.md) values directly as targets:
...
> [!NOTE]
> Text-based transactions built with the query builder are different from [interactive transactions](transactions.md). Text-based transactions execute all statements atomically in a single RPC call. Interactive transactions allow inspecting results between statements.

## Learn more

- [Executing queries](executing-queries.md) for running the built queries with `surrealdb.Query`
- [Data manipulation](data-manipulation.md) for the typed CRUD functions as an alternative to the query builder
