...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/data-models/full-text-search/analyzers-and-tokenizers.mdx"
---
...
These all decide a boundary from the characters around it, which is enough for languages that separate words with spaces or case changes. Chinese, Japanese and Korean need a dictionary instead: `segment(chinese | japanese | korean)`, documented under [DEFINE ANALYZER](../../../reference/query-language/statements/define/analyzer.md#segmentlanguage). Korean attaches grammatical particles to the word they follow, so `한국어를` only becomes searchable as `한국어` once it is segmented:
...
To add an analyzer only if it is missing, or to replace an existing definition, use `IF NOT EXISTS` or `OVERWRITE` on `DEFINE ANALYZER`. Examples and caveats are in the [statement reference](../../../reference/query-language/statements/define/analyzer.md#using-if-not-exists-clause).
