...
Use the [management API](../../reference/management-api.md) or `spectronctl` to create a Context per tenant, then mint an agent key with the appropriate scope floor:
...
Under scope-based isolation, a key with scope floor `{org: "alpha"}` physically cannot read data written by a key with scope floor `{org: "beta"}`. The server enforces this at the query level ([Scope as a security boundary](scope-as-security-boundary.md)).
...
- [Scope as a security boundary](scope-as-security-boundary.md) – how scope floors work
