...
A SurrealDB deployment that SurrealDB operates. SurrealDB handles provisioning, high availability, patching, backups, and version upgrades. See [Instances](../instances/index.md) for what that covers, and [Self-hosted](../self-hosted/index.md) if you would rather run the server yourself.
...
- [JavaScript and TypeScript](../../reference/javascript/index.md)
...
Any other language can use the [HTTP API](../instances/connect/via-http.md).

### Can I move an existing SurrealDB application onto a managed instance?

Yes. Export from your current deployment, then import into the instance. See [Import and export](../instances/import-and-export.md), and [Migrating to SurrealDB](../../build/migrating/index.md) for a move from another database.
...
The instance most likely has no user or access method defined yet. Studio authenticates with your own session, so it works before any credentials exist. An SDK needs credentials. See [Connect via SDK](../instances/connect/via-sdk.md#create-credentials).

### Are my instances backed up?

Yes. Automated snapshots run on a schedule, with tiered daily, weekly, and monthly retention. The snapshot frequency and the retention of each tier depend on the instance type, and Scale instances allow a configurable frequency. See [Backups and recovery](../instances/backups.md).
...
No. Deploy an instance in the target organisation, then move the data with [`surreal export` and `surreal import`](../instances/import-and-export.md).

### Can I delete an organisation?

Not at present. Delete the instances inside it, and it stops accruing charges. See [Organisations](index.md).

### What are the current limits?

- **Server flags and environment variables** cannot be set directly. [Instance capabilities](../instances/configure.md#capabilities) control runtime behaviour instead, in Studio or with `surrealctl instance capabilities`.
- **[GraphQL](../../learn/querying/graphql/overview.md)** is not yet enabled on managed instances.
...
- **Request sizes** are capped per endpoint. See [request size limits](../instances/connect/via-http.md#request-size-limits).

### Where are the documentation and tutorials?

- **[SurrealDB documentation](/docs):** Guides and reference, including [SurrealDB Studio](../../explore/studio/index.md).
...
Instances also inherit the [security features](../../learn/security/authentication/summary.md#product) of SurrealDB itself, and the [security practices](../../learn/security/authentication/summary.md#process) the database is developed under. To assess the service for a workload with specific requirements, use the [contact form](/contact).
...
- **[AWS PrivateLink](../instances/private-connectivity.md)** gives an instance a private network path from your AWS VPC. In `private` mode the instance gets no public hostname at all. It is available in a subset of regions, and the SurrealDB team onboards you to it.
- **[Instance capabilities](../instances/configure.md#capabilities)** restrict what the engine does: which RPC methods and HTTP endpoints it serves, whether guest access is permitted, and which [outbound destinations](../instances/network-access.md) a query can reach.

## Pricing

### How much does it cost?

You pay for the compute you consume and the storage you provision. Current rates are on the [pricing page](/pricing). [Usage](billing.md#usage-and-spend) shows what an organisation has accrued so far this month.
...
Studio prompts you to add [billing information](billing.md) to the organisation first.

### How am I billed?

Monthly, for the usage of the previous month. The cycle runs from the first day of a month to the last, and the invoice arrives by email. See [Billing](billing.md).
...
1. Confirm the instance is **running** and not [paused](../instances/configure.md#pause-an-instance). A paused instance is unreachable, and a free instance pauses itself after seven days.
...
4. On [PrivateLink](../instances/private-connectivity.md), confirm you connect from inside the VPC. A private hostname does not resolve outside it.
5. Check whether the failing operation depends on a denied [capability](../instances/configure.md#capabilities), such as an outbound HTTP call.
6. Check [metrics and logs](../instances/monitoring.md) for connection saturation or errors around the time the failures started.

If the connection still fails, raise a [support ticket](support.md) with the instance id.
