...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/spectron/self-hosting/operations/migrations-and-upgrades.mdx"
---
...
1. Take a SurrealDB backup of all Context databases (see [Backups and retention](backups-and-retention.md)).
...
Replace the `spectrond` binary (see [Bare metal](../deployment/bare-metal.md)), then restart the service:
...
Downgrade only when no **destructive** migration has been applied since your backup. If the newer version dropped or reshaped tables, run the restore workflow in [Backups and retention](backups-and-retention.md) rather than starting an older binary against the live database.
...
Store the backup externally. The full backup and restore workflow is in [Backups and retention](backups-and-retention.md).
