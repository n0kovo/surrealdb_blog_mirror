...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/php/v2/api/utilities.mdx"
---
...
| `events` | `?EventDispatcherInterface` | new dispatcher | PSR-14 dispatcher for [domain events](../concepts/events.md) |
| `logger` | `?LoggerInterface` | null logger | PSR-3 logger; enables the [logging middleware](../concepts/middleware.md#logging) |
| `tracer` | `?Tracer` | no-op tracer | Tracing backend; enables [telemetry](../concepts/observability.md) |
| `meter` | `?Meter` | no-op meter | Metrics backend; enables [telemetry](../concepts/observability.md) |
...
| `middleware` | `list<MiddlewareInterface>` | `[]` | Extra [middleware](../concepts/middleware.md) appended to the pipeline |
| `pingInterval` | `int` | `30` | WebSocket ping interval in seconds |

The `engines` and `*Factory` options are the transport-level extension seams. The [runtime presets](../concepts/runtimes.md) configure them for you, so you only set them when supplying a transport of your own.
...
A catalogue of features with the server version each requires. Pass one to [`isFeatureSupported()`](core.md#isfeaturesupported).
...
- [Core classes](core.md) for the `Surreal` entry point
- [Query builders](query-builders.md) for the fluent statement API
- [Executing queries](../concepts/executing-queries.md) for the guide
