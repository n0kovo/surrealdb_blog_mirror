...
Events let the database react to record changes: write an audit log, normalise related rows, or enqueue follow-up work. They run after the record change but inside the same transaction (unless you opt into async), and they see [$before and $after](../../../reference/query-language/language-primitives/parameters.md#before-after) snapshots of the record.
