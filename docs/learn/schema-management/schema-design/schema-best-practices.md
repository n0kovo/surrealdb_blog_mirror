...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/schema-management/schema-design/schema-best-practices.mdx"
---
...
When those definitions live in source control and need to roll out safely to shared databases, follow [SurrealKit schema migration](../../../manage/schema-migration/index.md) in the **Manage** section for sync, rollouts, testing in CI, and related tooling.

## Define arrays and sets with a type and length

In addition to a type, both arrays and sets can have a required number of items built into the type definition itself. A value with more or fewer items than the stated length is rejected. The definition below pairs this with an assertion using the [`array::all()`](../../../reference/query-language/functions/database-functions/array.md#arrayall) function to also ensure that every item in the `small_bytes` field is between 0 and 255.
...
Learn more about [database functions](../../../reference/query-language/functions/database-functions/index.md).
...
Inside a schemaless table, the only time an object will be schemafull is if the structure of the object is indicated using the [literal](../../../reference/query-language/language-primitives/data-types/literals.md) syntax.
...
That doesn't mean that you have to give up the precision of a `datetime` though. By using the [`time::format()`](../../../reference/query-language/language-primitives/formatters.md) function, you can keep the actual stored date as a precise SurrealQL `datetime` and then use that to output a string in any format you like.
...
If you find that parts of your table- or field-specific code are getting a bit long, it might be time to think about moving parts of it to a [database-wide parameter](../../../reference/query-language/statements/define/param.md).
...
Error types in programming languages often take the form of a long list of possible things that could go wrong. SurrealQL's [literal](../../../reference/query-language/language-primitives/data-types/literals.md) type allows you to specify a list of all possible forms it could take, which makes it well suited to error logic.
