...
Five keys are recognised. Each is outranked by an environment variable, and by a command-line flag above that; see the [precedence chain](../global-flags.md#the-precedence-chain).
...
Accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags). Column ids are `key`, `value`, `set` and `description`, with `kind` under `--wide`.
...
`credentials.json` lives beside it in the same directory. See [credential storage](../authentication.md#where-credentials-are-stored) for how that directory is chosen.
