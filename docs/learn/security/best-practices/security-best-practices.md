...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/security/best-practices/security-best-practices.mdx"
---
...
When running a SurrealDB server, you can configure the [capabilities](../authorization/capabilities.md) for your SurrealQL queries. Most of these capabilities are disabled by default to expose as little attack surface as possible to malicious actors.
...
When defining [users](../../../reference/query-language/statements/define/user.md) and [access methods](../../../reference/query-language/statements/define/access/index.md), ensure that you set a specific [session and token duration](../authentication/authentication.md#expiration) whenever possible using the `DURATION` clause.
...
When retrieving content that may be rendered in an HTML document, we strongly recommend that you use the [`string::html::encode`](../../../reference/query-language/functions/database-functions/string.md#stringhtmlencode) function, which will encode any characters that have special meaning in HTML syntax (e.g. `<`, `>`, `&`...) into HTML entities (e.g. `&lt;`, `&gt;`, `&amp;`...) that will be rendered as the actual original character instead of interpreted as HTML syntax.
...
The best way to protect tokens against stealing is to not store them at all. If your use case supports it, use the token in memory to [`authenticate`](../../../reference/javascript/api/core/surreal-session.md) a persistent session using the WebSocket protocol and destroy the token from memory after the session is established. When the session expires, ask your users to sign in again with their credentials and establish a new authenticated session with SurrealDB. Your use case may even support not using a token at all by directly authenticating the session with user credentials using [`signin`](../../../reference/javascript/api/core/surreal-session.md#signin).
