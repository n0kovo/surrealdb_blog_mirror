...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/querying/concepts-and-guides/testing.mdx"
---
...
Testing can also be done through direct SurrealQL queries to the database. While the CLI and SurrealDB Studio are the most convenient tools to use, sending in queries via the [HTTP endpoints](../../../reference/rest-api/http-protocol.md) may be more convenient when needing to test output depending on certain environments such as queries as a record user vs. as a database-level system user or a root user.

While you are exploring behaviour in SurrealQL, [`.expect()`](../../../reference/query-language/functions/database-functions/value.md#valueexpect) (*Since v3.1.0*) on intermediate results or at the end of a method chain makes it obvious which step failed before you wrap the same logic in an SDK test or a transaction with `THROW`.
...
One or more `.surql` files can be validated by the CLI using the [`surreal validate`](../../../reference/cli/surrealdb-cli/commands/validate.md) command.
