...
- **A downgrade is refused if the datastore has run a migration the older build does not ship.** The server reports the migration by name and stops. Reverting past a migration therefore needs [`surreal export`](../../reference/cli/surrealdb-cli/commands/export.md) and a reimport into a datastore created by the older version.
