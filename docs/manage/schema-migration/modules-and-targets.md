...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/manage/schema-migration/modules-and-targets.mdx"
---
...
A SurrealKit project applies one set of schema files to one database. That covers most projects and needs no configuration. When a project outgrows it, [`surrealkit.toml`](configuration.md) can declare **schema modules** - independently tracked sets of `.surql` files - and **targets** - named databases to apply them to.
...
Get the endpoint for a Cloud instance with [`surrealctl instance endpoint`](../../reference/cli/surrealctl/commands/instance.md).
...
- [Sync](sync.md): apply schema files to a database
- [Rollouts](rollouts.md): phased migrations for shared databases
