...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/querying/concepts-and-guides/idempotent-operations.mdx"
---
...
However, it can be made idempotent by first defining a [unique index](../../../reference/query-language/statements/define/indexes.md#composite-index) on the `in` and `out` fields. Doing so will result in the `RELATE` statement above always resulting in a single graph edge between the two records.
...
A duplicate explicit ID on `INSERT RELATION` returns an error unless you add `ON DUPLICATE KEY UPDATE`. For more, see [Explicit edge record IDs](../../../reference/query-language/statements/relate.md#handling-duplicate-edge-record-ids).
