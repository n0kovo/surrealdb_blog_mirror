...
The functions it governs are the [HTTP functions](../../reference/query-language/functions/database-functions/http.md) that call an API, fetch a document, or post to a webhook as part of a statement.

> [!IMPORTANT]
> This is outbound access. It has nothing to do with who can reach the instance. For inbound connectivity, see [Private connectivity](private-connectivity.md) and its `public`, `private`, and `dual` access modes.
...
> [!NOTE]
> Configurable network access requires SurrealDB `>=2.1.8, <2.2.0`, `>=2.2.6, <2.3.0`, or `>=2.3.6`. On an earlier release, [upgrade the instance](versions-and-upgrades.md) before you configure the rules.
...
- **[HTTP functions](../../reference/query-language/functions/database-functions/http.md):** the functions this capability governs.
