...
A `RecordId` uniquely identifies a record in SurrealDB. It consists of a table name and an ID value. The ID can be a `long`, `String`, `UUID`, or a composite key using `Array` or `Object`. See the [SurrealQL record ID documentation](../../../query-language/language-primitives/data-types/record-ids.md) for details on how record identifiers work in SurrealDB.
