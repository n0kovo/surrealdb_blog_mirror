...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/rust/concepts/fetch.mdx"
---
...
The following example shows a classroom joined to a few students by [record links](../../query-language/language-primitives/record-links.md).
...
[The `.*` operator](../../query-language/language-primitives/idioms.md#all-elements) for the `teacher` and `student` fields can be used in this case.
...
[The `.*` operator](../../query-language/language-primitives/idioms.md#all-elements) for the `teacher` and `student` fields can be used in this case, but note that the `.*` must be used twice in the case of the students: once to access each member of the array, and once more to access all of its fields.
