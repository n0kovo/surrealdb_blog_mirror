...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/query-language/language-primitives/data-types/objects.mdx"
---
...
As a record is essentially an object with a required [`id` field](record-ids.md) that can be created, updated, or deleted, they can be worked with in almost exactly the same way as a standalone object.
...
When you combine backticks with dotted paths for nested fields, keep any periods used to denote a nested path **outside** the backticks. Anything inside backticks is treated as a single literal field name, including a `.` inside the backticks. For definitions and concrete examples (including non-ASCII identifiers), see [`DEFINE FIELD`](../../statements/define/field.md#example-usage).
...
* [Object functions](../../functions/database-functions/object.md)
* [Destructuring nested objects](../idioms.md#destructuring)
