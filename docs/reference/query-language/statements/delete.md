...
```
                     ...
                     │ │            │ │                  │                                   │ │                 │                             │ │ │                                    │ │                  │            │ │               
        ╭────────╮   │ │  ╭──────╮  │ │   ┌──────────┐   │    ╭───────╮     ┌────────────┐   │ │    ╭────────╮   │         ╭────────╮          │ │ │    ╭─────────╮     ┌───────────┐   │ │    ╭─────────╮   │  ╭──────╮  │ │   ╭───╮       
├┼──────│ DELETE │───╯─╯──│ ONLY │──╰─╰───│ @targets │───╯────│ WHERE │─────│ @condition │───╰─╯────│ RETURN │───┼─────────│ BEFORE │──────────┼─╰─╯────│ TIMEOUT │─────│ @duration │───╰─╯────│ EXPLAIN │───╯──│ FULL │──╰─╰───│ ; │─────┼┤
        ╰────────╯        ╰──────╯        └──────────┘        ╰───────╯     └────────────┘          ╰────────╯   │         ╰────────╯          │        ╰─────────╯     └───────────┘          ╰─────────╯      ╰──────╯        ╰───╯       
                                                                                                                 │                             │                                                                                            
                                                                                                                 │          ╭───────╮          │                                                                                            
                                                                                                                 ╰──────────│ AFTER │──────────╯                                                                                            
...
An important point to know when using a `WHERE` clause is that it performs a check on the [truthiness](../language-primitives/data-types/values.md#values-and-truthiness) of a value, namely whether a value exists and is not a default value like 0, an empty string, empty array, and so on.
...
This pattern is particularly useful when using SurrealDB's [literal types](../language-primitives/data-types/literals.md). A literal type containing objects that contain a single top-level field can easily be matched on through the field name.
...
While soft deletions do not exist natively in SurrealDB, they can be simulated by [defining an event](define/event.md) that reacts whenever a deletion occurs.

The following example archives the data of a deleted record in another table. This can be combined with  [fewer permissions for the new table](define/table.md#defining-permissions) so that it can be accessed only by [system users](define/user.md) and not [record users](define/access/record.md).
