...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/javascript/concepts/connecting-to-surrealdb.mdx"
---
...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/javascript/api/core/surreal#connect"> ` db.connect(url, options) `</a></td>
			<td scope="row" data-label="Description">Connects to a local or remote database endpoint</td>
		</tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/languages/javascript/api/core/surreal#close"> ` db.close() `</a></td>
            <td scope="row" data-label="Description">Closes the persistent connection to the database</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/languages/javascript/api/core/surreal#ready"> ` db.ready `</a></td>
			<td scope="row" data-label="Description">Waits for the connection to the database to succeed</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/javascript/api/core/surreal-session#use"> ` db.use(namespace, database)`</a></td>
...
You can directly specify the [namespace](../../../reference/query-language/statements/define/namespace.md) and [database](../../../reference/query-language/statements/define/database.md) to use using the `namespace` and `database` options. If you do not specify these options, the default namespace and database will be used.
Once the connection is established, you can switch the active namespace and database with the `.use()` method.

#### Authentication details

When connecting as a [system user](../../../learn/security/authentication/authentication.md#system-user) or [token](../../../learn/security/authentication/authentication.md#token), you can directly pass your credentials to the `authentication` option.
...
You can extend the session duration of a token or a session by setting the `DURATION` clause when creating a new access method with the [`DEFINE ACCESS METHOD`](../../../reference/query-language/statements/define/access/index.md) statement, or when defining a new user with the [`DEFINE USER`](../../../reference/query-language/statements/define/user.md) statement.
Learn more about token and session duration in our [security best practices](../../../learn/security/best-practices/security-best-practices.md#expiration) documentation.
...
- [Surreal API reference](../api/core/surreal.md) for the complete connection interface
- [ConnectOptions type reference](../api/types/index.md#connectoptions) for all connection options
- [RetryOptions type reference](../api/types/index.md#retryoptions) for retry-on-conflict configuration
- [Authentication](authentication.md) for signing in and managing credentials
- [WebAssembly engine](../engines/wasm.md) for embedded browser databases
- [Node.js engine](../engines/node.md) for embedded server-side databases
- [Error handling](error-handling.md) for connection and reconnection errors
- [Security best practices](../../../learn/security/best-practices/security-best-practices.md) for production deployments
