...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/query-language/language-primitives/data-types/sets.mdx"
---
...
Casting into a `set` and back into an array can be a convenient way to deduplicate items in the same way that the [`array::distinct()`](../../functions/database-functions/array.md#arraydistinct) and [`array::sort()`](../../functions/database-functions/array.md#arraysort) functions are used.
...
To return the greatest value of a set, use `[$]` (since 3.2.0) or the [`set::last()`](../../functions/database-functions/set.md#setlast) function.
...
When the field has a [`VALUE`](../../statements/define/field.md#using-the-value-clause-to-set-a-fields-value) clause on its items, that clause is applied before the set removes duplicates. Two entries that only differ in a way the clause normalises therefore collapse into one on the write that adds them.
