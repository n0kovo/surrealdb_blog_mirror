...
Web framework state is where this can add up. A router stores its state by cloning it into every route, and an extractor clones it again on each request, so a bare `Surreal<C>` passed to framework functions such as Axum's `with_state`, Actix's `app_data` or Rocket's `manage` opens a session per route and another per request. Wrap the client in an `Arc` first, or hold it in a static as the [framework guides](../frameworks/index.md) do.
