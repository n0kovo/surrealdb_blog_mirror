...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/agent-memory/cookbooks/patterns/historical-and-archaeological-data.mdx"
---
...
Upload records as documents rather than as many synchronous facts. See [Documents (upload)](../../reference/rest-api.md#documents-upload).
...
- **Leave similar names alone.** `Holconius`, `Marcus Holconius`, and `Marcus Holconius Priscus` may be one person or three. Let resolution and [reconciliation](https://surrealdb.com/docs/agent-memory/reasoning/reconciliation-and-supersession) decide. A manual merge destroys the evidence for the decision.
...
Reads then accept `asOf`. You can ask what the corpus attests for one moment rather than for all time. See [Temporal validity](https://surrealdb.com/docs/agent-memory/reasoning/temporal-validity) and [spoiler-safe narrative memory](spoiler-safe-narrative-memory.md) for more on this subject.
...
`k` on `/query` defaults to **10** and deployments cap it at **50**. The internal retrieval pool is set separately from `k`. See [Answer size vs search breadth](https://surrealdb.com/docs/agent-memory/retrieve/hybrid-search#answer-size-vs-search-breadth).
...
- **Turn on query expansion for document retrieval.** `/documents/query` accepts `useHyde` and `decomposeQuery`. Both help when query wording and corpus wording diverge. See [Hybrid search](https://surrealdb.com/docs/agent-memory/retrieve/hybrid-search#advanced-options).
...
3. **`/reflect`** for interpretive claims across many records. Reflection always calls a model. It returns synthesised text with its evidence. See [Reflection](https://surrealdb.com/docs/agent-memory/operations/reflect).
...
- **Page correctly when you list documents.** Follow `page.nextCursor` from `GET /documents?limit=100` until it is absent, rather than stopping on the first short page. See [Pagination](../../reference/rest-api.md#pagination).
...
- **Use a fresh Context per experiment.** Operator questions and prior answers become hits, so a reused Context contaminates the next run. See [Contexts and scope](https://surrealdb.com/docs/agent-memory/mental-model/contexts-and-scope).
