...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/manage/schema-migration/library/example.mdx"
---
...
For the macro-based approach to sync, see the [`embed_schema!` macro](../embed-schema-macro.md).
...
Use sync against local or ephemeral databases where fast iteration matters and pruning is safe. Schema files are embedded at compile time with `include_str!`, then applied with the [`Sync`](index.md) builder.
...
Use rollouts for shared and production databases. Build a [`RolloutSpec`](index.md) with the builder, wrap it in the `Rollout` facade, then call `start` and `complete` at the appropriate points around your application deployment.
...
If a process is killed mid-rollout, the `__rollout` record can be left in a `running_*` state. Inspect it with `rollout.status(&db).await?` and recover it with the CLI [`rollout repair`](../rollouts.md) command or `Rollout::abandon(&db, "20260604__add_account").await?`.
</content>
