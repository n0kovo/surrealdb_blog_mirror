...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/agent-memory/integrations/mcp-server/tools-reference.mdx"
---
...
Operation failures return **`isError: true`** tool results with `structuredContent.error.status` mirroring REST (404, 403, 429, 401, 500). JSON-RPC **`error`** is reserved for protocol faults (bad params, unknown tool). Auth and missing-Context failures are masked as **401**. See [MCP tools — error handling](../../reference/mcp-tools.md#error-handling).
