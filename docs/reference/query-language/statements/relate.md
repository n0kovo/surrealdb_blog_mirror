...
However, clauses available in [`SELECT` statements](select.md) such as `WHERE`, `LIMIT`, `GROUP BY`, aliases and so on can be used, making a graph clause a most flexible option.
