...
| No provenance | Every record carries a **`source`** object (kind, ref, spans, trust, derivation); see [Provenance and traceability](https://surrealdb.com/docs/agent-memory/mental-model/provenance-and-traceability) |
| No correction tracking | Supersession chains plus explicit **`uncertainty`** for cross-provenance clashes |
| No temporal validity | [Tri-temporal](https://surrealdb.com/docs/agent-memory/architecture/tri-temporal-model) model (system, known, and valid time) |
| No verifiability | **Traces** (`retrieval_trace`, `decision_trace`, `response_trace`) as substrate nodes, not disposable logs |
| Unreliable retrieval | [Hybrid structural retrieval plus tiered resolution](https://surrealdb.com/docs/agent-memory/architecture/coherence-retrieval-and-tiers) |

The result is a memory layer **you can trust**: correctness can be demonstrated, not just assumed. See [The accuracy promise](https://surrealdb.com/docs/agent-memory/welcome/accuracy-promise) for how that works in practice.
