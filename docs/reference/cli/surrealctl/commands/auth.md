...
Behaviour when nothing is signed in is deliberate and differs by verb: `status` and `logout` succeed with exit `0`, while `scopes` and `refresh` exit `3`. See [Authentication](../authentication.md#what-each-auth-verb-does-when-nothing-is-signed-in) for why.
...
For the API's own view of your identity, use [`whoami`](misc.md#whoami), which spends a request.
...
The command forces both the access token and the Cloud session. `--all` additionally renews the token used for the account-management routes, which is the one the [`token` commands](token.md) need.
...
For the catalogue of scopes a token *can* be granted, use [`token scopes`](token.md#token-scopes).
