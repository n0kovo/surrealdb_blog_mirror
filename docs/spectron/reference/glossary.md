...
The precedence rule: **Authoritative** (curated) content takes precedence over **Experiential** assertions when they conflict. Disagreements are surfaced via **`uncertainty`** records and conflict indicators in state/profile responses — curated records are not silently overwritten. See [Authority when pillars meet](../agent-memory/reasoning/authority-hierarchy.md).
...
The unit of memory storage and configuration isolation in Spectron. Each Context is bound to a SurrealDB `(namespace, database)` pair and has its own API keys, model configuration, and data. Contexts cannot see each other's data. See [Contexts and scope](https://surrealdb.com/docs/spectron/mental-model/contexts-and-scope).
...
Hierarchical scope paths (for example `org/acme/user/alice/`) that partition data within a Context. Records store visibility as **OR-of-AND clauses** (each clause lists scope nodes that must all apply together). Queries and grants resolve which clauses a caller can see. See [Contexts and scope](https://surrealdb.com/docs/spectron/mental-model/contexts-and-scope).
...
The **Experiential** pillar covers conversational input and derived facts (`turn`, `reflect`, `elaboration`, `consolidation`, …). It includes the **six [memory categories](https://surrealdb.com/docs/spectron/mental-model/memory-categories)**: episodic session/turn data plus identity, knowledge, context, instructions, and uncertainty.
...
The ability to trace every record back to its **source** (turn, document, or trace). Records carry a structured **`source`** object (`kind`, `ref`, `span`, `trust`, `derived_from`, …). Some examples use `source_turn` as shorthand for conversational `source.ref`. See [Provenance and traceability](https://surrealdb.com/docs/spectron/mental-model/provenance-and-traceability).

## Reconciliation

The **Reconciliation** pillar: integrating newly extracted information with existing beliefs – deduplication, supersession, scope coexistence, and **Authoritative** precedence. See [Eight pillars and six categories](https://surrealdb.com/docs/spectron/architecture/eight-pillars-and-categories).
