...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/golang/concepts/error-handling.mdx"
---
...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Error type"><a href="/docs/languages/golang/api/errors#servererror">`ServerError`</a></td>
			<td scope="row" data-label="Description">Structured error from SurrealDB v3 with kind, details, and cause chain</td>
		</tr>
		<tr>
			<td scope="row" data-label="Error type"><a href="/docs/languages/golang/api/errors#queryerror">`QueryError`</a></td>
			<td scope="row" data-label="Description">Per-statement query failure returned within QueryResult</td>
		</tr>
		<tr>
			<td scope="row" data-label="Error type"><a href="/docs/languages/golang/api/errors#rpcerror">`RPCError`</a></td>
...
SurrealDB v3 returns structured errors with a `Kind`, `Message`, `Details`, and an optional `Cause` chain. Use `errors.As` to extract a [`ServerError`](../api/errors/index.md#servererror):
...
When using `Query`, individual statements can fail without causing the entire call to error. Each [`QueryResult`](../api/types/index.md#queryresult) has an `Error` field that contains a [`QueryError`](../api/errors/index.md#queryerror) if that statement failed.
...
The SDK defines several [sentinel errors](../api/errors/index.md#sentinel-errors) for common failure conditions:
...
- [Errors API reference](../api/errors/index.md) for complete error type definitions
- [Executing queries](executing-queries.md) for query-level error handling
- [Types reference](../api/types/index.md) for `QueryResult` and `QueryError` definitions
