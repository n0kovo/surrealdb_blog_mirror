...
`--org` overrides it for a single command, and `SURREALCTL_ORG` overrides it for a shell. See [Organisations](organisations.md#choosing-an-organisation) for the full order of precedence.
...
For the equivalent views in the browser, see [Monitoring](../instances/monitoring.md).
...
See [Backups and recovery](../instances/backups.md) for retention behaviour and what a snapshot contains.
...
For what each capability means, see [Configure an instance](../instances/configure.md) and [Network access](../instances/network-access.md).
...
- [Organisations](organisations.md) — members, roles, invitations, usage, and spend.
- [Scripting](scripting.md) — exit codes and the `--json` contract in full.
- [`surrealctl` reference](../../reference/cli/surrealctl/overview.md) — every instance flag and default.
