...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/manage/self-hosted/docker.mdx"
---
...
SurrealDB Enterprise is published as a separate image, [`surrealdb/surrealdb-enterprise`](https://hub.docker.com/r/surrealdb/surrealdb-enterprise), whose entrypoint is the Enterprise server binary, so it takes the same commands as the Community image. It needs a [licence key](../enterprise/index.md#licence-key) to start. The following command passes the key from the `SURREAL_LICENSE_KEY` variable in the current shell:
