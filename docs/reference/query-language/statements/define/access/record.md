...
> [!NOTE]
> Due to changes required in the RPC API and the SDKs, refresh tokens are currently only available when signing up and in through the [HTTP REST API](../../../../rest-api/http-protocol.md).
...
Sign up with a new user or sign in with an existing user via the [HTTP REST API](../../../../rest-api/http-protocol.md):
