...
Since SurrealDB 3.0, in-memory storage runs on a backend called [SurrealMX](https://github.com/surrealdb/surrealmx) that also allows for [versioned queries](../reference/query-language/statements/select.md#the-version-clause) and [persistent storage](../reference/cli/surrealdb-cli/commands/start.md#supported-parameters-for-memory-surrealmx) as a snapshot or AOL (append-only log).
