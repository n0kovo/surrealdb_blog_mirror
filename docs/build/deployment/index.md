...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/build/deployment/index.mdx"
---

# Deployment

SurrealDB separates the query engine (compute) from the underlying storage layer. The same SurrealQL, APIs, and client SDKs work across embedded devices, single-node servers, distributed clusters, and [SurrealDB Cloud](surrealdb-cloud/what-is-surrealdb-cloud.md) — so you can change how the database runs without rewriting application code.
...
See also [Self-hosted deployment](self-hosted/overview.md) for Docker, Kubernetes, and platform guides.
...
> [!NOTE]
> **SurrealDS has not yet reached general availability.** The distributed storage layer described below is in development for <Edition value="enterprise" />. For managed multi-node clusters today, use [SurrealDB Cloud](surrealdb-cloud/what-is-surrealdb-cloud.md) Dedicated tiers.
...
When SurrealDS is available, operational signals will be documented under [Enterprise observability](../../manage/observability/enterprise-observability.md) (including the `surrealdb.ds.*` metric family).
...
Distributed deployments add cluster orchestration, node management, monitoring, and replication management. Teams that want distributed scalability without operating that stack should consider [SurrealDB Cloud](surrealdb-cloud/what-is-surrealdb-cloud.md).
...
For language-specific setup, see [Embedding SurrealDB](../embedding/index.md) and [Storage engines](../embedding/storage-engines.md).
...
Until SurrealDS is available, use [SurrealDB Cloud](surrealdb-cloud/what-is-surrealdb-cloud.md) Dedicated tiers or contact SurrealDB for Enterprise early-access programmes.
...
- **[SurrealDB Cloud](surrealdb-cloud/what-is-surrealdb-cloud.md)** — Provision a managed SurrealDB instance with built-in monitoring, network access controls and operations tooling.
- **[Self-hosted](self-hosted/overview.md)** — Run SurrealDB on Docker, Kubernetes (AKS, EKS, GKE) or as a standalone binary with full control over storage and configuration.
- **[Observability](../../manage/observability/index.md)** — Metrics, OTLP and Prometheus access, audit logs and slow-query logs for both Community and Enterprise editions.
