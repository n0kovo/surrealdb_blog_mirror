...
SurrealDB is designed to run in many different ways and environments. Due to the [separation of the storage and compute layers](architecture.md), you can use it as an embedded database, a vertically scalable single-node server, or a horizontally scalable multi-node cluster — with the same SurrealQL and APIs throughout. In-memory operation (including [SurrealMX](running/in-memory.md)), browser persistence via [IndexedDB](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API), on-disk [RocksDB](running/file-backed.md) for write-heavy server deployments, and optional beta [SurrealKV](running/file-backed.md) for embedded and local-first deployments cover most local and single-node cases. For distributed production, use [SurrealDB Cloud](build/deployment/surrealdb-cloud/what-is-surrealdb-cloud.md) or self-hosted highly available clusters on [Amazon EKS](build/deployment/self-hosted/amazon-eks.md), [Google GKE](build/deployment/self-hosted/google-gke.md), and [Azure AKS](build/deployment/self-hosted/azure-aks.md). See [Deployment](build/deployment/index.md) for a full comparison and guide.
