...
Where the per-statement outcome matters, [`query_raw()`](../api/core/surreal.md#query-raw) returns every statement instead of raising. Each entry carries a `status` of `OK` or `ERR`, and a failing one also carries the `kind`.
...
Every server error carries a `.kind`, and the SDK raises a dedicated class for each of the kinds below. The meaning of each kind is described in [Errors](../../rest-api/errors.md#error-kinds). Match on the kind or the class rather than on the message text, which is free to change between releases.
