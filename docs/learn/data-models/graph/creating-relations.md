...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/data-models/graph/creating-relations.mdx"
---

# Creating relations

In SurrealDB, nodes are typically records in ordinary tables (users, posts, companies, and so on). Graph edges are also **real tables**, created and queried with [`RELATE`](../../../reference/query-language/statements/relate.md).

## Creating nodes and edge records

SurrealDB uses [`RELATE`](../../../reference/query-language/statements/relate.md) to form the usual graph triple: subject → predicate → object.
...
That shows how to read fields on the edge table directly. For traversing with arrow syntax from nodes, see [Graph traversal](graph-traversal.md).
...
Once the edge exists, you can traverse it with the arrow operator (forward, backward, recursively and more). See [Graph traversal](graph-traversal.md) for patterns such as:
...
Adding `TYPE RELATION` to [`DEFINE TABLE`](../../../reference/query-language/statements/define/table.md) is enough to enforce that behaviour.
...
Strict relation definitions also feed [SurrealDB Studio](../../../explore/studio/index.md)’s Designer view.
...
This can be used as an ad-hoc unique index, especially if the ID itself contains the IDs of the records being linked. The [`array::sort()`](../../../reference/query-language/functions/database-functions/array.md#arraysort) function can also be used here for symmetric relations.
...
Querying symmetric edges with `<->` is covered in [Graph traversal](graph-traversal.md). For friendship-oriented examples, see [Social network patterns](social-network-patterns.md).
...
To forbid that, add [`ENFORCED`](../../../reference/query-language/statements/define/table.md) on the relation table:
...
Some workflows rely on creating the edge first (for example, a street with predictable house IDs before the houses exist). A [`DEFINE FIELD ... VALUE`](../../../reference/query-language/statements/define/field.md) can describe the path from `house` to `street` and resolve once the house exists:
