...
Personal access tokens are read-only here: the mutating verbs answer 403, exit `4`. See [Authentication](../authentication.md#what-a-personal-access-token-cannot-do).
...
Also accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags). Column ids are `name`, `id`, `plan`, `state`, `role` and `members`, with `billing_provider`, `max_free`, `max_paid`, `privatelink`, `locked` and `archived_at` shown only under `--wide`.
...
`org use` remembers an organisation *within* a profile. [`context use`](context.md#context-use) switches the whole profile.
...
Also accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags). Column ids are `name` and `permissions`.

```bash
surrealctl org roles
```

The role vocabulary is per organisation, which is why [`team invite`](team.md#team-invite) and [`team update`](team.md#team-update) point here rather than validating a `--role` value locally.
...
Also accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags). Column ids are `resource` and `action`.

```bash
surrealctl org permissions --org acme
```

This is the authorisation model to consult when a command answers 403. [`auth scopes`](auth.md#auth-scopes) reports the credential's own scopes, which are a different thing.
...
Also accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags). Column ids are `instance_id`, `metric_type`, `compute_hours`, `disk_used_bytes`, `period_start` and `period_end`, with `instance_type` and `source` under `--wide`.

```bash title="Compute hours per instance, largest first"
surrealctl org usage --sort compute_hours --reverse
```

For one instance rather than all of them, use [`instance usage`](instance.md#instance-usage).
...
Also accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags). Column ids are `description`, `resource`, `amount`, `units` and `effective_at`, with `instance_id`, `instance_type` and `cloud_usage_units` under `--wide`.
...
Also accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags). Column ids are `name`, `id`, `regions` and `instance_types`, with `description` under `--wide`.

```bash
surrealctl org plans
```

This is the organisation-scoped answer, and the one to trust before an [`instance create`](instance.md#instance-create). The [`catalog`](catalog.md) commands list what the platform offers globally, which is always the wider set.
