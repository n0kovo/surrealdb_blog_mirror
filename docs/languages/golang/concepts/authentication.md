...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/golang/concepts/authentication.mdx"
---
...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/golang/api/core/db#signin">`db.SignIn(ctx, authData)`</a></td>
			<td scope="row" data-label="Description">Signs in an existing user and returns a JWT token</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/golang/api/core/db#signinwithrefresh">`db.SignInWithRefresh(ctx, authData)`</a></td>
			<td scope="row" data-label="Description">Signs in and returns both an access token and a refresh token</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/golang/api/core/db#signup">`db.SignUp(ctx, authData)`</a></td>
			<td scope="row" data-label="Description">Signs up a new record user and returns a JWT token</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/golang/api/core/db#signupwithrefresh">`db.SignUpWithRefresh(ctx, authData)`</a></td>
			<td scope="row" data-label="Description">Signs up a new record user and returns both tokens</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/golang/api/core/db#authenticate">`db.Authenticate(ctx, token)`</a></td>
			<td scope="row" data-label="Description">Authenticates the connection with a JWT token</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/golang/api/core/db#invalidate">`db.Invalidate(ctx)`</a></td>
			<td scope="row" data-label="Description">Invalidates the current authentication</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/golang/api/core/db#info">`db.Info(ctx)`</a></td>
...
You can provide credentials using either the [`Auth`](../api/types/index.md#auth) struct or a `map[string]any`.
...
Record-level authentication requires the `Access` field, which specifies which [`DEFINE ACCESS`](../../../reference/query-language/statements/define/access/index.md) method to use.
...
The returned [`Tokens`](../api/types/index.md#tokens) contains an `Access` field (JWT) and a `Refresh` field. To obtain new tokens without re-entering credentials, pass the refresh token:
...
- [Types reference](../api/types/index.md) for `Auth` and `Tokens` type definitions
- [Connecting to SurrealDB](connecting-to-surrealdb.md) for connection protocols and their effect on authentication
- [DEFINE ACCESS statement](../../../reference/query-language/statements/define/access/index.md) for configuring access methods
- [Security best practices](../../../learn/security/best-practices/security-best-practices.md) for token and session duration configuration
