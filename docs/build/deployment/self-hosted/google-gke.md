...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/build/deployment/self-hosted/google-gke.mdx"
---
...
> [!NOTE]
> For a **single-node RocksDB** deployment on Kubernetes (including a cluster you create with GKE), start with [Deploy on Kubernetes](kubernetes.md). For a local open-source distributed-storage playground only, see [Run a multi-node cluster](../../../running/multi-node.md).
...
| Single SurrealDB pod, RocksDB on a volume | [Deploy on Kubernetes](kubernetes.md) on a GKE cluster you manage |
