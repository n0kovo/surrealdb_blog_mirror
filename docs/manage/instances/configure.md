...
**Allow and deny lists** apply to capabilities with many members: RPC methods, HTTP endpoints, functions, and [network access](network-access.md). Each list starts from a default and takes exceptions:
...
> [!IMPORTANT]
> **Network access** governs outbound HTTP requests made from inside queries, and is denied by default. It does not control who can reach the instance. See [Network access](network-access.md) for the pattern syntax, and [Private connectivity](private-connectivity.md) for inbound access modes.

The **Import** and **Export** buttons read and write the whole capability configuration as a JSON file. Use the export to keep staging and production aligned, or to hold the configuration in version control. `surrealctl instance capabilities` does the same from a terminal. See [surrealctl instances](../surrealctl/instances.md).
...
Expect a brief reconnect while the instance restarts on the new size. For when to resize and what to watch afterwards, see [Scaling](scaling.md).
...
> [!WARNING]
> Do not treat the snapshots of an instance as the safety net for deleting it. If you might need the data later, [restore a backup into a new instance](backups.md) or take a [logical export](import-and-export.md) first. Confirm the export loads cleanly before you delete.
...
- **[Monitoring](monitoring.md):** confirm a configuration change had the effect you expected.
- **[Backups and recovery](backups.md):** snapshot frequency, retention, and restore.
