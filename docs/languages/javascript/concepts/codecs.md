...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/javascript/concepts/codecs.mdx"
---
...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Codec">`CborCodec`</td>
			<td scope="row" data-label="Wire format">`Uint8Array` (CBOR with SurrealDB tags)</td>
		...
		<tr>
			<td scope="row" data-label="Codec">`JsonCodec`</td>
...
The CBOR layout uses SurrealDB's tagged values. See the [CBOR protocol reference](../../../reference/rest-api/cbor-protocol.md) for the tag list.
...
<table>
	...
	<tbody>
		<tr>
			...
			<td>`CborCodec` (used automatically by the SDK)</td>
		</tr>
		<tr>
			<td>Storing query results in a JSON document store</td>
			<td>`JsonCodec` or `jsonify()` for string representations</td>
		</tr>
		<tr>
			<td>Logging or debugging typed values</td>
			<td>`JsonCodec` or `jsonify()`</td>
		</tr>
		<tr>
		...
		<tr>
			<td>Browser storage (`localStorage`, IndexedDB as JSON)</td>
			<td>`JsonCodec`</td>
		</tr>
	</tbody>
</table>

For a plain string form (good for display or simple serialisation), [`jsonify()`](utilities.md#jsonifying-query-results) converts value instances to SurrealQL strings without the SQON JSON wrappers.
...
<table>
	...
	<tbody>
		<tr>
			<td>`useNativeDates`</td>
			<td>Decode datetimes as native `Date` instead of `DateTime` (loses nanosecond precision)</td>
		</tr>
		<tr>
			<td>`valueEncodeVisitor`</td>
			<td>Custom function applied to each value before encoding</td>
		</tr>
		<tr>
			<td>`valueDecodeVisitor`</td>
...
- [CBOR protocol reference](../../../reference/rest-api/cbor-protocol.md) for the full CBOR tag specification
- [Value types](value-types.md) for the value classes codecs operate on
- [Utilities](utilities.md) for `jsonify()` and other SQON helpers
