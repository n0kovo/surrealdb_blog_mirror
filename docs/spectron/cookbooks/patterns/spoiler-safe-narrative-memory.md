...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/spectron/cookbooks/patterns/spoiler-safe-narrative-memory.mdx"
---
...
See [Temporal validity](../../agent-memory/reasoning/temporal-validity.md#narrative-playback-and-spoiler-safety) for the model; this page is the recipe.
...
- **`asOf` on `/query`** walks known-time on attributes and relations; pair with [Temporal validity](../../agent-memory/reasoning/temporal-validity.md) for `valid_from` / `valid_until` on in-world dates.
- Response caching is bypassed when temporal filters are present — correct for narrative playback, slightly higher latency.

## Related reading

- [Storing memories](../../agent-memory/ingest/experiential/remember.md) — `observed_at` and `--as-of` on the CLI
- [Uploading documents](../../agent-memory/ingest/authoritative/uploading-documents.md) — `observedAt` in upload metadata
- [Recalling memories](../../agent-memory/retrieve/recall.md) — `asOf` and `occurredAt` on hits
- [Contexts and scope](https://surrealdb.com/docs/spectron/mental-model/contexts-and-scope) — `lens` and labels
