...
![Diagram of the single-node topology: one SurrealDB compute node paired with its own dedicated disk-based storage volume.](../../assets/img/image/cloud/light/start-single-node-light.png)

This topology suits development, staging, and production workloads that fit on one node and can tolerate a short recovery window. It scales vertically: as load grows, you move to a larger instance type. See [Scaling](scaling.md).

Because there is a single node, there is no failover inside the instance. [Backups](backups.md) are the recovery mechanism. See [High availability](high-availability.md) for what that means in practice.
...
- **Self-hosting the same topology is more work.** Running it yourself means operating Kubernetes, storage replication, patching, backups, and upgrade orchestration. Scale covers that work for you. Compare [Self-hosted deployment models](../self-hosted/deployment-models.md).
...
- **[Scaling](scaling.md):** resizing nodes and adding them.
- **[High availability](high-availability.md):** what each topology survives.
- **[Self-hosted deployment models](../self-hosted/deployment-models.md):** running the same topologies on your own infrastructure.
