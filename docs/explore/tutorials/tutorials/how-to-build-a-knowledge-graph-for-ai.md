...
Want to give it a try yourself? Head on over to the [online Surrealist UI](https://app.surrealdb.com/), go into the sandbox and run the following statements to set up the schema and seed data before running the query we just saw. To keep that data, use **Deploy to Cloud** in Surrealist to create a free SurrealDB Cloud instance, then connect to it and run the same statements there.
