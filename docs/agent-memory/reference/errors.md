...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/agent-memory/reference/errors.mdx"
---
...
Poll `GET /api/v1/{context_id}/documents/{id}` to check document status. See [Uploading documents](https://surrealdb.com/docs/agent-memory/ingest/authoritative/uploading-documents) for retry guidance.
...
Auth failures and missing Contexts are masked as **401**, never 404, so unauthenticated callers cannot enumerate Context ids. See [MCP tools](mcp-tools.md#error-handling).
