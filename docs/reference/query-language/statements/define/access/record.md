...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/query-language/statements/define/access/record.mdx"
---
...
- You must be authenticated as a [root, namespace or database user](../user.md) before you can define a record access method.
- [You must select your namespace and database](../../use.md) before you can define a record access method.
...
> [!IMPORTANT]
> For `TYPE RECORD ... WITH JWT`, the `id` claim must identify the record user, unless an [`AUTHENTICATE`](#with-authenticate-clause) clause resolves that record from other claims (for example an email address). This differs from [`TYPE JWT`](jwt.md#using-tokens), where `id` is not required and the session is a system user.
...
Because refresh tokens can be used to indefinitely keep a user authenticated with SurrealDB as long as they are exchanged for a new fresh token before they expire, [special care](../../../../../learn/security/best-practices/security-best-practices.md#token-storage) should be taken when storing and applications using them should be suitably protected from attacks.

Like other bearer keys, all refresh tokens are stored in the datastore even after they are expired or revoked. This means that using refresh tokens will have a space cost in addition to the performance cost of retrieving and verifying them against the datastore. Refresh tokens are intended to be used only to obtain a new authentication token after the existing one expires and applications should only use them when necessary, such as after receiving a [token expiration error](../../../../../learn/security/best-practices/troubleshooting.md#token-expired-error). For certain high volume applications, you may want to regularly [purge](../../access.md#purge) expired refresh tokens to minimize the space used by inactive refresh tokens.

For more information on how to manage existing refresh tokens, see the [`ACCESS`](../../access.md) statement.
...
Sign up with a new user or sign in with an existing user via the [HTTP REST API](../../../../rest-api/http-protocol.md). The RPC `signup` and `signin` methods return the same `access` and `refresh` pair as their result:
...
Unlike the [`PERMISSIONS`](../table.md#defining-permissions) clause, the `AUTHENTICATE` clause is executed only at the time of authentication, resulting in increased performance for queries that only need to be validated at that point.
