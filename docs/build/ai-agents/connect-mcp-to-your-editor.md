...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/build/ai-agents/connect-mcp-to-your-editor.mdx"
---
...
For the full tool list, limits, and security model, see [Model Context Protocol (MCP)](mcp.md). This page focuses on getting a working setup you can verify in a few minutes.

> [!NOTE]
> SurrealDB 3.1+ includes MCP in the main binary. Third-party MCP servers listed on [Surreal Labs](../../labs/index.md) (for example [surrealdb-mcp-server on GitHub](https://github.com/nsxdavid/surrealdb-mcp-server)) are separate projects; this tutorial uses the **built-in** server only.

## Choose your setup

Both `surreal start` (to start a SurrealDB server) and `surreal mcp` (to start a SurrealDB MCP server) expose the same tools (`query`, `select`, `use`, …). Pick based on **who shares the database** and **how auth should work**. See [When to use `surreal mcp` vs `surreal start`](mcp.md#when-to-use-surreal-mcp-vs-surreal-start) for the full comparison.
...
> [!WARNING]
> Stdio MCP runs every tool call with owner-level access. Use it only on a machine you trust. Do not expose `surreal mcp` to untrusted users. See the [security checklist](mcp.md#security-checklist) before production HTTP deployments.

## Prerequisites

1. [Install SurrealDB](../../running/installation/index.md) and confirm the binary is on your `PATH`.
...
Replace the header value with Base64-encoded credentials for your SurrealDB user (for example `root:secret` during local dev). In production, prefer a least-privilege `DEFINE USER` and TLS in front of the server — see [MCP security](mcp.md#security-checklist).
...
Once connected, the assistant can use the published tools documented on the [MCP overview](mcp.md#published-tools). Typical workflows:
...
Permissions match those for the [`/sql`](../../reference/rest-api/http-protocol.md#post-sql) endpoint: table `PERMISSIONS`, user roles, and server capability flags all apply.
...
| `403 Forbidden: Host header is not allowed` (HTTP) | The `/mcp` transport only accepts loopback hosts by default. Set `SURREAL_MCP_ALLOWED_HOSTS` to your FQDN (include `localhost` if you still need it), or `SURREAL_MCP_ALLOW_ALL_HOSTS=true` behind a trusted proxy. Requires SurrealDB 3.2.1+. See [MCP configuration](mcp.md#configuration). |
| Query timeout | Default tool timeout is 60 seconds (`SURREAL_MCP_QUERY_TIMEOUT_SECS`). Large scans may need a narrower query. |
| Result truncated | Output is capped (default 256 KiB). Paginate or aggregate in SurrealQL. See [MCP configuration](mcp.md#configuration). |
...
- [Model Context Protocol (MCP)](mcp.md) — transports, tools, environment variables, security
- [`surreal mcp` CLI reference](../../reference/cli/surrealdb-cli/commands/mcp.md)
- [Agent Skills](agent-skills.md) — teach assistants SurrealQL patterns alongside MCP
- [Why SurrealDB for AI agents](index.md) — memory, retrieval, and agent architecture
