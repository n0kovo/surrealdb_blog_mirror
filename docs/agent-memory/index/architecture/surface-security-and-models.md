...
Trace listing: `GET /api/v1/{ctx}/traces`, `GET /api/v1/{ctx}/traces/stats` (supports **`windowHours`** for rolling operational aggregates), and `GET /api/v1/{ctx}/traces/{id}`. See [Traces and memory evolution](https://surrealdb.com/docs/agent-memory/architecture/traces-and-evolution) and [REST API](../../reference/rest-api.md).
...
- **Prompt-injection handling** – ingest-time scanning on uploads and batched turns; untrusted text is sanitised before it is interpolated into extraction, chat, and consolidation prompts (see [Extraction pipeline](https://surrealdb.com/docs/agent-memory/reasoning/extraction-pipeline#untrusted-metadata-in-prompts)).
...
See [Traces and memory evolution](https://surrealdb.com/docs/agent-memory/architecture/traces-and-evolution) for more details.
