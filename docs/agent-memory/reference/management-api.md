...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/agent-memory/reference/management-api.mdx"
---
...
Top-level fields such as **`enforcement_blocked`** (pay-as-you-go vs hard block - see [Configuration](configuration.md#top-level-context-fields-outside-config)) may be set on the same `PATCH` body outside `config`.
...
Document ingestion requires a management key or an agent key with explicit upload capability. See [Uploading documents](https://surrealdb.com/docs/agent-memory/ingest/authoritative/uploading-documents) for the full upload API.
...
List endpoints page with `limit`, `cursor` and `count`, and return the collection beside a `page` block. The contract is identical to the end-user surface, so see [Pagination](rest-api.md#pagination) for the parameters, the response shape, and the rule about not terminating on a short page.
