...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/query-language/language-primitives/data-types/booleans.mdx"
---
...
When performing a query on the database, accessing a record's ID directly or using a [record range](record-ids.md#record-ranges) allows performance to be significantly sped up by avoiding the table scan which a `WHERE` clause on an unindexed field needs.
...
All SurrealQL values are either truthy or not. While seemingly similar to booleans in that `true` is a truthy value and `false` is not, the truthiness of a value extends to all value types and is based on the existence of a concrete value as opposed to empty values, `NONE`, `NULL`, and so on. For more information and examples, see [this page](values.md#values-and-truthiness).
