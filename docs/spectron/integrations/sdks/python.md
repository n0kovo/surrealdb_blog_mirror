...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/spectron/integrations/sdks/python.mdx"
---
...
On the wire, scope is a **ScopeSet**: an ordered array of slash-path strings (for example `["org/acme/user/alice"]`). Register paths with `spectron scopes create` before first use; see [Contexts and scope](https://surrealdb.com/docs/spectron/mental-model/contexts-and-scope).
...
→ Full method tables: [Python SDK reference](../../reference/sdk-python.md)
...
The default timeout is 30 seconds; streaming chat disables the read timeout while tokens arrive. On a 429, read the problem-detail `body` and back off before retrying manually. All errors follow [RFC 7807 Problem Details](../../reference/errors.md).
