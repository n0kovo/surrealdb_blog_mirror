...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/rust/methods/connect.mdx"
---
...
gRPC is the only remote protocol that delivers query results incrementally, which is what makes [`.stream_items()`](query.md#stream-items) worthwhile on a remote connection. On WebSocket and HTTP the results are buffered and replayed instead.
...
A gRPC message has a size limit at each end. The server sets its own with [`SURREAL_GRPC_MAX_MESSAGE_SIZE`](../../cli/surrealdb-cli/environment-variables.md) and advertises it on connection, and the SDK takes that advertised figure by default. `Config::grpc` sets the client's own figure, for a workload that writes a large batch in one transaction:
...
* [Streaming query results with `.stream_items()`](query.md#stream-items)
