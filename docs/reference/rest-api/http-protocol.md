...
The GQL endpoint runs **read-only** [ISO GQL (OpenGQL)](../../learn/querying/gql/overview.md) graph pattern queries (`MATCH … RETURN …`) against your existing tables and `RELATE` edges.

> [!WARNING]
> OpenGQL is **experimental**. Enable it at server start with [`--allow-experimental=opengql`](../cli/surrealdb-cli/commands/start.md#experimental-capabilities) (or `SURREAL_CAPS_ALLOW_EXPERIMENTAL=opengql`). `--allow-all` does **not** enable it. If the capability is off, expect **403**.
...
Parse errors return **HTTP 400** with an error payload. See [GQL via HTTP](../../learn/querying/gql/via-http.md) for enabling OpenGQL, seeding data, and RPC examples.
