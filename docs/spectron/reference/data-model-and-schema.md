...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/spectron/reference/data-model-and-schema.mdx"
---
...
Scoped tables carry **`scope_sets`** — content-addressed OR-of-AND clauses referencing hierarchical **`scopes`** paths. Entity and keyword index records also store derived **`visible_scope_sets`** so name/type lookups cannot leak facts the caller cannot read whole. See [Contexts and scope](https://surrealdb.com/docs/spectron/mental-model/contexts-and-scope) for the visibility rules; the snippets below use simplified `scope` field names where the wire shape is still `key=value` tags in examples.
...
Migrations are applied once per Context database. Spectron tracks applied migrations in a `_migrations` table in each Context database. See [Migrations and upgrades](../self-hosting/operations/migrations-and-upgrades.md).
