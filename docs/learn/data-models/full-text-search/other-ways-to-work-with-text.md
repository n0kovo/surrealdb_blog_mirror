...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/data-models/full-text-search/other-ways-to-work-with-text.mdx"
---
...
To sort strings in a more natural manner to the human eye, the keywords [`NUMERIC` and `COLLATE` (or both) can be used](../../../reference/query-language/statements/select.md#sort-records-using-the-order-by-clause). `NUMERIC` will instruct strings that parse into numbers to be treated as such.
...
SurrealDB has a number of [operators](../../../reference/query-language/language-primitives/operators.md) to determine if all or some of the values of one array are contained in another, such as `CONTAINSALL` and `CONTAINSANY`, or `ALLINSIDE` and `ANYINSIDE`. The queries with `CONTAINS` and `INSIDE` perform the same operation, just in the opposite order.
...
SurrealDB has functions that combine full-text and vector search results. For more on that pattern, see [Hybrid search](../vector-search/hybrid-search.md) in the vector search section.
...
- [Search functions](../../../reference/query-language/functions/database-functions/search.md)
- [SurrealDB search functions](../../../reference/query-language/functions/database-functions/search.md)
- [SurrealDB operators](../../../reference/query-language/language-primitives/operators.md)
