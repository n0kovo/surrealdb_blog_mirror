...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/query-language/language-primitives/data-types/literals.mdx"
---
...
Literals can be defined inside a database schema by using a [DEFINE FIELD](../../statements/define/field.md) statement.
...
Next, we will [define a function](../../statements/define/function.md) to handle this field and return a certain type of message depending on the error. Note the following:

* The `LET` statement in the first line is simply to shorten the path to the information contained inside `error_info`
* `IF ELSE` statement works here because [IF](../../statements/throw.md) involves a check for [truthiness](values.md#values-and-truthiness), returning `true` as long as it finds a value that is not none, empty, or zero.
