...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                Idle lifetime of a persisted HTTP RPC session before expiry (default `24h`)
            </td>
        </tr>
        <tr>
            <td colspan="2">
            ...
            <td>
                Interval for purging expired durable HTTP RPC sessions (default `60s`; `0` disables the sweep)
            </td>
        </tr>
    </tbody>
</table>
...
Since SurrealDB 3.0, strict mode is now enacted on the database level via a [`DEFINE DATABASE`](../../../query-language/statements/define/database.md) statement.
...
While SurrealKV supports historical/temporal querying using the `VERSION` clause when [selecting](../../../query-language/statements/select.md#the-version-clause) or [creating](../../../query-language/statements/create.md#version) data, you must explicitly opt in to this using the `surrealkv+versioned://` prefix in the path argument.
...
Client-attached sessions on the HTTP [`/rpc`](../../../rest-api/rpc-protocol.md) endpoint normally live only in the process memory of the node that created them. After a restart, or when a later request is routed to a different node, the client receives a session-not-found error.
