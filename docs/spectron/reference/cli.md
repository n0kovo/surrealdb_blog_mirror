...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/spectron/reference/cli.mdx"
---
...
For a full provisioning walkthrough (bootstrap, scopes, principals, keys, upload, query), follow the [Self-hosted quickstart](https://surrealdb.com/docs/spectron/quickstarts/self-hosted) and [CLI](#connection-flags-client) reference below.
...
The MCP server is served at `/mcp` on the same host and port as the REST API — no CLI step is needed. Point your client at that endpoint, or run [`install-mcp`](https://github.com/supermemoryai/install-mcp). See the [MCP server install guide](../integrations/mcp-server/install.md).
