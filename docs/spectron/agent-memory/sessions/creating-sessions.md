...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/spectron/agent-memory/sessions/creating-sessions.mdx"
---
...
As messages flow through your application, record each one by posting to the turns endpoint. Spectron extracts structured facts from each turn synchronously and returns the extraction result immediately. See [Adding turns](adding-turns.md) for the full extraction result shape.
...
The ownership restriction applies to turn submission only. Reading the state and retrieving context from a session may be permitted to other principals depending on the access control configuration of the context. See [Authentication and principals](../../self-hosting/security/principals.md) for details.
