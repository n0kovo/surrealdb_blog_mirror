...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/agent-memory/integrations/frameworks/hermes.mdx"
---
...
> [!NOTE]
> This is the hosted provider. For a persistent filesystem on a SurrealDB instance you run, with a toolset the agent calls directly, see the [SurrealDB filesystem integration for Hermes](../../../build/integrations/ai-frameworks/hermes.md). Only one memory provider can be active at a time, so pick one.
...
- For an MCP-native host, use the [MCP server](../mcp-server/install.md).
- To call SurrealDB Agent Memory directly, use the [Python SDK](../sdks/python.md).
