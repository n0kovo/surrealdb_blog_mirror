...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/query-language/statements/define/api.mdx"
---
...
A defined API has access to a preset parameter called [`$request`](../../language-primitives/parameters.md#request) which contains the request sent by the caller of the endpoint. The $request parameter may contain values at the following fields: `body`, `headers`, `params`, `method`, `query`, and `context`.
...
```
                                 ...
                                 │ │                                        │ │                   │                                       │ │                                           │ │                                                  │ │                      │                   │ │     
        ╭────────╮     ╭─────╮   │ │    ╭────╮     ╭─────╮     ╭────────╮   │ │   ┌───────────┐   │    ╭─────╮     ┌──────────────────┐   │ │    ╭────────────╮     ┌───────────────┐   │ │    ╭──────╮     ╭───╮     ┌────────┐     ╭───╮   │ │    ╭─────────────╮   │     ╭──────╮      │ │     
├┼──────│ DEFINE │─────│ API │───╯─╯────│ IF │─────│ NOT │─────│ EXISTS │───╰─╰───│ @endpoint │───╯────│ FOR │─────│ @HTTP_method, .. │───╰─╯────│ MIDDLEWARE │─────│ @function, .. │───╰─╯────│ THEN │─────│ { │─────│ @value │─────│ } │───╰─╯────│ PERMISSIONS │───┼─────│ FULL │──────┼─╰───┼┤
        ╰────────╯     ╰─────╯          ╰────╯     ╰─────╯     ╰────────╯         └───────────┘        ╰─────╯     └──────────────────┘          ╰────────────╯     └───────────────┘          ╰──────╯     ╰───╯     └────────┘     ╰───╯          ╰─────────────╯   │     ╰──────╯      │       
                                                                                                                                                                                                                                                                      │                   │       
                                                                                                                                                                                                                                                                      │  ┌─────────────┐  │       
                                                                                                                                                                                                                                                                      ╰──│ @expression │──╯       
...
`DEFINE API` is often used in conjunction with a [capabilities flag](../../../../learn/security/authorization/capabilities.md) or [environment variable](../../../cli/surrealdb-cli/environment-variables.md) to disable arbitrary queries, thereby forcing record and anonymous users to interact with the database via API endpoints alone.
...
An API endpoint can be tested using the [`api::invoke` function](../../functions/database-functions/api.md), which takes either the path as a single string or the path along with a request body. It can also be tested via [CURL or other means by directly using the endpoint](../../../rest-api/http-protocol.md) along with the namespace and database in the headers.
...
A custom middleware function is defined in the same way as any other [user-defined function](function.md).
