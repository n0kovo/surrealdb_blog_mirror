...
`auth status` and `auth scopes` touch no network at all, so they work on a plane, in a container with no egress, and in a CI job that is about to fail for a different reason. [`whoami`](commands/misc.md#whoami) is the command that asks the API who you are.
...
- [`auth` commands](commands/auth.md) — every flag on every verb
- [`token` commands](commands/token.md) — creating and revoking tokens
- [Global flags](global-flags.md) — `--token`, `--token-file`, `--profile`
- [Overview](overview.md) — the rest of the reference
- [SurrealDB CLI](../surrealdb-cli/overview.md) — database credentials, which are a separate concern from these
