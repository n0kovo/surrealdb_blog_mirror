...
`surrealkit typegen` introspects a live database and emits a structured description of its schema. JSON is the primary output; TypeScript interfaces for the [SurrealDB JavaScript SDK](../../reference/javascript/index.md) can be generated alongside it.
...
Type generation is also available from the [`surrealkit::typegen`](library/index.md) module when embedding SurrealKit in Rust:
...
- [Sync](sync.md): keep the database aligned with your schema files
- [Using SurrealKit as a library](library/index.md): drive SurrealKit from Rust
</content>
