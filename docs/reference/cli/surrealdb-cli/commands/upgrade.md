...
[`surrealctl`](../../surrealctl/overview.md) can download a copy of the `surreal` binary for you when a command such as `surrealctl instance sql` needs it. That copy lives in `~/.config/surrealctl/bin/` and is only used when no `surreal` binary is found on `PATH`, so a version installed normally always takes precedence.
