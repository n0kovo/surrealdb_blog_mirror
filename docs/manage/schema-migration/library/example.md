...
If a process is killed mid-rollout, the `__rollout` row can be left in a `running_*` state. Inspect it with `rollout.status(&db).await?` and recover it with the CLI [`rollout repair`](../rollouts.md) command or `Rollout::abandon(&db, "20260604__add_account").await?`.
</content>
