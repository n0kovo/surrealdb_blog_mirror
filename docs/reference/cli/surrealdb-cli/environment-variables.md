...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_TABLE_DOC_IDS_BATCH_SIZE`*Since v3.3.0*</td>
      <td scope="row" data-label="Default">1000</td>
      <td scope="row" data-label="Allowed values">A u32</td>
      <td scope="row" data-label="Notes">The number of document ids allocated at once from a table's shared document id sequence, which full-text, HNSW and DiskANN indexes use. Larger batches reduce coordination on the distributed sequence, at the cost of larger gaps in the ids when a node is lost before it uses up its current batch. Replaces `SURREAL_FTS_DOC_IDS_BATCH_SIZE`, which 3.3.0 no longer reads.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_GRAPH_FOLD_BATCH`*Since v3.3.0*</td>
      <td scope="row" data-label="Default">1024</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">How many graph adjacency keys one background fold pass packs into blocks per transaction. This bounds the transaction size on a vertex with many edges, which is then folded over several passes. See `SURREAL_GRAPH_FOLD_INTERVAL`.</td>
    </tr>
  </tbody>
</table>
...
<tr>
    ...
    <tr>
      ...
      <td scope="row" data-label="Default">268,435,456 (256 MiB)</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">The maximum total size, in bytes, of the DISKANN index cache, shared across all DISKANN indexes in the process. DISKANN graph data lives in the key-value store and is paged through this bounded cache. The DISKANN counterpart of `SURREAL_HNSW_CACHE_SIZE`.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_GRAPH_RESOLVE_CACHE_SIZE`*Since v3.3.0*</td>
      <td scope="row" data-label="Default">67,108,864 (64 MiB)</td>
      <td scope="row" data-label="Allowed values">A u64</td>
      <td scope="row" data-label="Notes">The maximum total size, in bytes, of the cache that maps document ids back to record keys when a traversal reads numeric-encoded graph adjacency entries. Only relevant once `SURREAL_GRAPH_NUMERIC_IDS` has written numeric blocks.</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tr>
    ...
    <tr>
      ...
      <td scope="row" data-label="Default">1000</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">The maximum number of idle HTTP connections to maintain. Has no effect: earlier versions read the value without applying it to the HTTP client, and 3.3.0 no longer reads it. Use `SURREAL_MAX_HTTP_IDLE_CONNECTIONS_PER_HOST` instead.</td>
    </tr>
<tr>
...
<tr>
    ...
    <tr>
      ...
      <td scope="row" data-label="Notes">From 3.3.0. The largest single gRPC message the server will accept or emit, also reported to clients as `Limits.max_message_bytes`. This is the gRPC counterpart of `SURREAL_WEBSOCKET_MAX_MESSAGE_SIZE`: raise it for a transaction that writes a large batch in one request.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_GRPC_MAX_ATTACHED_SESSIONS`*Since v3.3.0*</td>
    ...
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_GRPC_NOTIFICATION_BUFFER`*Since v3.3.0*</td>
      <td scope="row" data-label="Default">1024</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">How many live query notifications can be buffered for a gRPC subscription whose client is not keeping up. When the buffer is full, the server ends that subscription with a `RESOURCE_EXHAUSTED` status instead of waiting.</td>
    </tr>
    <tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_BITMAP_BRANCH_BUDGET`*Since v3.3.0*</td>
      <td scope="row" data-label="Default">250,000</td>
      <td scope="row" data-label="Allowed values">A usize; `0` disables the budget</td>
      <td scope="row" data-label="Notes">How many index entries one range branch of a bitmap plan, which combines several indexes into a bitmap of candidate records, may read before that branch is abandoned. The abandoned branch's condition is then checked as an ordinary `WHERE` filter, so an unselective range cannot make the bitmap plan slower than scanning. The same budget bounds the allow-list built for a pre-filtered vector search (see <a href="#vector-search-config">Vector search config</a>).</td>
    </tr>
    <tr>
    ...
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_FAN_OUT_ROW_THRESHOLD`*Since v3.3.0*</td>
    ...
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_MAX_EXPRESSION_PARSING_DEPTH`*Since v3.3.0*</td>
      <td scope="row" data-label="Default">128</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">How deep the parser will build an expression operator tree. This bounds long operator chains such as `1 + 1 + 1 + …` and prefix/postfix chains, which consume neither of the other two parsing budgets.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_MAX_OBJECT_PARSING_DEPTH`</td>
      <td scope="row" data-label="Default">100</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">How deep the parser will parse nested objects and arrays. Type annotations such as `array&lt;option&lt;array&lt;int&gt;&gt;&gt;` count against it as well. For data arriving over an SDK or RPC this is the governing limit. For a query written as text, see <a href="#nesting-depth-in-queries-versus-data">Nesting depth in queries versus data</a>.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">How deep the parser will parse recursive queries (queries within queries). Object and array literals written into a query count against this budget as well as against `SURREAL_MAX_OBJECT_PARSING_DEPTH`, and this one is the smaller of the two - see <a href="#nesting-depth-in-queries-versus-data">Nesting depth in queries versus data</a>.</td>
    </tr>
    <tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_SURREALISM_SILO_ENDPOINT`*Since v3.3.0*</td>
      <td scope="row" data-label="Default">`https://silo.surrealdb.com`</td>
      <td scope="row" data-label="Allowed values">A URL</td>
      <td scope="row" data-label="Notes">The base URL that a `silo::` module executable in <a href="/docs/reference/query-language/statements/define/module">DEFINE MODULE</a> is fetched from, as `{endpoint}/{organisation}/{package}/{major}.{minor}.{patch}.surli`. A trailing slash is removed. Set it to a mirror to serve packages from somewhere else.</td>
    </tr>
  </tbody>
</table>
...
These variables tune pre-filtered vector search. When a KNN query on an HNSW or DiskANN index has `WHERE` conditions that other indexes can answer, SurrealDB first evaluates those conditions into an allow-list of matching records and restricts the vector search to it. See [How the filter is applied](../../../learn/data-models/vector-search/similarity-search.md#how-the-filter-is-applied).
...
<table>
  ...
  <tbody>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_KNN_PREFILTER_ENABLED`</td>
      <td scope="row" data-label="Default">true</td>
      <td scope="row" data-label="Allowed values">true, false</td>
      <td scope="row" data-label="Notes">Whether KNN queries build an allow-list from their `WHERE` conditions. When `false`, the conditions are checked for each candidate during the index traversal instead, as before 3.3.0.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_KNN_PREFILTER_EXACT_THRESHOLD`</td>
    ...
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_KNN_PREFILTER_EF_BOOST`</td>
      <td scope="row" data-label="Default">4</td>
      <td scope="row" data-label="Allowed values">A u32</td>
      <td scope="row" data-label="Notes">The multiplier applied to the search width (`ef`) of a traversal restricted to an allow-list. A selective allow-list rejects most candidates, and a wider search recovers the recall that loses. The result is capped by `SURREAL_KNN_PREFILTER_EF_MAX` and is never lower than the `ef` the query asks for.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_KNN_PREFILTER_EF_BOOST_THRESHOLD`</td>
      <td scope="row" data-label="Default">100,000</td>
      <td scope="row" data-label="Allowed values">A u64</td>
      <td scope="row" data-label="Notes">The allow-list size above which the traversal stops multiplying `ef`, because a large allow-list rejects few candidates. Results are still restricted to the allow-list at every size.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_KNN_PREFILTER_EF_MAX`</td>
      <td scope="row" data-label="Default">1024</td>
      <td scope="row" data-label="Allowed values">A u32</td>
      <td scope="row" data-label="Notes">The ceiling for the multiplied `ef`. It never lowers the `ef` a query asks for.</td>
    </tr>
  </tbody>
</table>

The allow-list is built within the entry budget set by [`SURREAL_BITMAP_BRANCH_BUDGET`](#limits-config). When the budget is exceeded, the query falls back to checking the conditions during the traversal. A vector index created before 3.3.0 uses the allow-list only after it is rebuilt with [`REBUILD INDEX`](../../query-language/statements/rebuild.md).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_WEBSOCKET_MAX_CONCURRENT_STREAMS`*Since v3.3.0*</td>
      <td scope="row" data-label="Default">32</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">Maximum number of streaming queries (the `query_stream` RPC method) running at once on a single WebSocket connection. Each open stream holds an executing query, with its read snapshot or transaction, for as long as the client keeps reading. A request over the limit is refused with an error rather than queued.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_WEBSOCKET_STREAM_SEND_TIMEOUT`*Since v3.3.0*</td>
      <td scope="row" data-label="Default">30</td>
      <td scope="row" data-label="Allowed values">Seconds (integer), clamped to 1-86,400</td>
      <td scope="row" data-label="Notes">How long a streaming query may wait to hand one frame to the connection before the stream is stopped. The wait resets for every frame, so it limits a stalled client rather than the length of a stream, and it applies even when `SURREAL_QUERY_TIMEOUT` is not set.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_MAX_TRANSACTIONS_PER_CONNECTION`*Since v3.2.2*</td>
      <td scope="row" data-label="Default">64</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">Maximum concurrently open client-managed transactions (`begin` RPC) on a WebSocket connection's implicit default session. Further `begin` calls return `Too many open transactions` until a slot is freed by `commit`, `cancel`, `reset`, or disconnect. Does not apply to attached sessions (see `SURREAL_MAX_TRANSACTIONS_PER_SESSION`).</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_MAX_TRANSACTIONS_PER_SESSION`*Since v3.2.2*</td>
      <td scope="row" data-label="Default">64</td>
      <td scope="row" data-label="Allowed values">A usize</td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td scope="row" data-label="Allowed values">A duration</td>
      <td scope="row" data-label="Notes">Version retention period as a duration string. Used by the rocksdb and surrealkv engines. From 3.3.0 the memory engine refuses a non-zero value and the server does not start. *Since v3.3.0*</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td scope="row" data-label="Notes">Whether MVCC versioning is enabled, for use with the <a href="/docs/reference/query-language/statements/select#the-version-clause">`VERSION` clause</a>. Used by the rocksdb and surrealkv engines. From 3.3.0 the memory engine does not support versioned queries, and refuses `true` so that the server does not start. *Since v3.3.0*</td>
    </tr>
    <tr>
...
<table>
  ...
  <tr>
      ...
      <td scope="row" data-label="Command arg">`async-event-interval`</td>
...
<tr>
    ...
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_DURABLE_SESSIONS`*Since v3.2.2*</td>
      <td scope="row" data-label="Command arg">`durable-sessions`</td>
      <td scope="row" data-label="Command">`start`</td>
      <td scope="row" data-label="Default">false</td>
      <td scope="row" data-label="Allowed values">true, false</td>
    ...
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_DURABLE_SESSION_GC_INTERVAL`*Since v3.2.2*</td>
      <td scope="row" data-label="Command arg">`durable-session-gc-interval`</td>
      <td scope="row" data-label="Command">`start`</td>
      <td scope="row" data-label="Default">60s</td>
      <td scope="row" data-label="Allowed values">A duration; `0` disables the background sweep</td>
    ...
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_DURABLE_SESSION_TTL`*Since v3.2.2*</td>
      <td scope="row" data-label="Command arg">`durable-session-ttl`</td>
      <td scope="row" data-label="Command">`start`</td>
    ...
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_GRAPH_FOLD_INTERVAL`*Since v3.3.0*</td>
      <td scope="row" data-label="Command arg">`graph-fold-interval`</td>
      <td scope="row" data-label="Command">`start`</td>
      <td scope="row" data-label="Default">5s</td>
      <td scope="row" data-label="Allowed values">A duration</td>
      <td scope="row" data-label="Notes">The interval between background passes that fold a vertex's per-edge graph adjacency keys into packed blocks. A pass also starts early when a write queues fold work or a traversal meets a vertex with many unfolded keys. See also `SURREAL_GRAPH_FOLD_BATCH`.</td>
    </tr>
    <tr>
...
`slow-log-param-allow` *Since v2.3.9*</td>
    ...
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_STARTUP_OPERATION_TIMEOUT`</td>
      <td scope="row" data-label="Command arg">`startup-operation-timeout`</td>
      <td scope="row" data-label="Command">`start`</td>
      <td scope="row" data-label="Default">60s</td>
      <td scope="row" data-label="Allowed values">A duration</td>
...
<table>
  ...
  <tbody>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_HTTP_AUTH_RATE_LIMIT_ENABLED`</td>
      <td scope="row" data-label="Default">false</td>
      <td scope="row" data-label="Allowed values">true, false</td>
      <td scope="row" data-label="Notes">Whether to rate limit authentication attempts on the HTTP `/signin` and `/signup` endpoints per client address. Only `true` and `false` are accepted; any other value is ignored and the default applies.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_HTTP_AUTH_RATE_LIMIT_BURST`</td>
      <td scope="row" data-label="Default">60</td>
      <td scope="row" data-label="Allowed values">A u32</td>
      <td scope="row" data-label="Notes">How many authentication attempts a single client address may make before being throttled. Set to `0` to disable the limiter.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_HTTP_AUTH_RATE_LIMIT_PER_MINUTE`</td>
      <td scope="row" data-label="Default">60</td>
      <td scope="row" data-label="Allowed values">A u32</td>
      <td scope="row" data-label="Notes">The sustained rate, in attempts per minute, at which a client address's budget refills. The default of 60 is one attempt per second. Set to `0` to disable the limiter.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var">`SURREAL_HTTP_AUTH_RATE_LIMIT_MAX_TRACKED_CLIENTS`</td>
      <td scope="row" data-label="Default">16384</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">How many client addresses the limiter tracks at once. Unauthenticated callers write to this store, so it is a bounded cache: at capacity a new address evicts an existing entry rather than the store growing or the limiter switching off. An evicted address starts again from a full burst, so the sustained rate binds an address only while it stays tracked. Size this above the number of distinct addresses the instance authenticates. Set to `0` to disable the limiter.</td>
    </tr>
  </tbody>
</table>
