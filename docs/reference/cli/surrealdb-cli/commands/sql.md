...
Both `surreal sql` and [`surreal start`](start.md) accept the same capability flags (`--allow-funcs`, `--allow-eval-query`, `--allow-experimental`, and so on). **Where those flags take effect depends on how you connect.**
...
For a one-shot query without staying in the REPL, pipe SurrealQL into `surreal sql`. This is only for a small number of statements; for larger scripts use the [import command](import.md).
