...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/data-models/full-text-search/scoring-and-ranking.mdx"
---
...
To use highlighting and best match scoring on searches, the `BM25` and `HIGHLIGHTS` clauses can be added to the `DEFINE INDEX` statement. These enable you use the [`search::highlight`](../../../reference/query-language/functions/database-functions/search.md#searchhighlight) and [`search::score`](../../../reference/query-language/functions/database-functions/search.md#searchscore) functions.
...
In addition to full-text search capabilities, SurrealDB has a number of other ways of working with text such as string similarity, regex matching, and functions that order text in different ways. See [Other ways to work with text](other-ways-to-work-with-text.md).
