...
> [!NOTE]
> This guide deploys a **highly available cluster** on AKS and connects SurrealDB to it. For a **single-node RocksDB** deployment on Kubernetes, start with [Deploy on Kubernetes](kubernetes.md). For managed deployments, see [SurrealDB Cloud](../surrealdb-cloud/what-is-surrealdb-cloud.md).
...
> [!NOTE]
> This guide was tested in `westeurope` region and follows best practices for scalability and high availability.
...
4. After creation finishes, get credentials to configure `kubectl` to connect to the new cluster:
