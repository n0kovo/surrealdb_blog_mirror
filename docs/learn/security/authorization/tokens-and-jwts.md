...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/security/authorization/tokens-and-jwts.mdx"
---
...
You register JWT-based access with **`DEFINE ACCESS … TYPE JWT`** (see [`DEFINE ACCESS`](../../../reference/query-language/statements/define/access/index.md)). SurrealDB validates the token’s signature and standard claims (such as issuer and audience) according to your definition. Verification supports **symmetric** algorithms such as **HS256** with a shared secret, and **asymmetric** algorithms such as **RS256** and **ES256** using a configured public key or JWKS—matching how major providers sign tokens at scale. Choose the model that matches your key management: shared secrets for internal services, public-key verification when keys are rotated by an external IdP.
...
- How claim names map into session data (so **`$auth`** exposes fields your [`PERMISSIONS`](permissions-and-row-level-security.md) rules can use).

**Claims** from the JWT are mapped into SurrealDB **session variables** (including **`$auth`**) so [permissions](permissions-and-row-level-security.md) and queries see a consistent view of the user whether authentication came from SurrealDB or from elsewhere. Typical mappings include subject, roles, and custom namespaces your application relies on for **authorisation**. The exact claim names depend on your issuer; configure the mapping in `DEFINE ACCESS` so your policies read stable fields regardless of whether the upstream token uses flat or nested JSON.
...
In practice, teams often start with **HS256** and a single secret for development or private networks, then move to **RS256** or **ES256** with JWKS in production so verification does not depend on long-lived shared secrets crossing service boundaries. The [`DEFINE ACCESS`](../../../reference/query-language/statements/define/access/index.md) reference lists the exact clauses for each approach.

For how authentication fits the wider security model, read the [authentication overview](../authentication/overview.md). For every option on JWT and record access, use the reference for [`DEFINE ACCESS`](../../../reference/query-language/statements/define/access/index.md).
