...
The [local database serving](../../explore/surrealist/concepts/local-database-serving.md) functionality on the Surrealist UI can also be used to start a server.
...
> [!WARNING]
> GQL is **experimental**. Enable it at server start with [`--allow-experimental gql`](../cli/surrealdb-cli/commands/start.md#experimental-capabilities) (or `SURREAL_CAPS_ALLOW_EXPERIMENTAL=gql`). `--allow-all` does **not** enable it. If the capability is off, expect **403**.
