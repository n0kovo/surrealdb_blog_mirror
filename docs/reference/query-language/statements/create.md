...
```
                     ...
                     │            │                  │ │                                          │ │ │                 │                                         │ │ │                                    │               
        ╭────────╮   │  ╭──────╮  │   ┌──────────┐   │ │    ╭─────╮     ┌─────────────────────┐   │ │ │    ╭────────╮   │               ╭────────╮                │ │ │    ╭─────────╮     ┌───────────┐   │   ╭───╮       
├┼──────│ CREATE │───╯──│ ONLY │──╰───│ @targets │───╯─╯────│ SET │─────│ @field = @value ... │───╰─╰─╯────│ RETURN │───┼───────────────│ BEFORE │────────────────┼─╰─╯────│ TIMEOUT │─────│ @duration │───╰───│ ; │─────┼┤
        ╰────────╯      ╰──────╯      └──────────┘          ╰─────╯     └─────────────────────┘            ╰────────╯   │               ╰────────╯                │        ╰─────────╯     └───────────┘       ╰───╯       
                                                                                                                        │                                         │                                                        
                                                                                                                        │                ╭───────╮                │                                                        
                                                                                                                        ╰────────────────│ AFTER │────────────────╯                                                        
...
If you are using [SurrealKV as the storage engine](../../../architecture.md) with versioning enabled, when creating a record you can specify a version for each record. This is useful for time-travel queries. You can query a specific version of a record by using the `VERSION` clause.

The `VERSION` clause is always followed by a [datetime](../language-primitives/data-types/datetimes.md) and when the specified timestamp does not exist, an empty array is returned.
...
This is to allow the chance to [define](define/database.md) them manually, such as by including a comment.
...
To disallow this behaviour, you can [define a database](define/database.md) using the `STRICT` keyword. In strict mode, any resource must first be explicitly defined before it can be used.
