...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/query-language/language-primitives/data-types/ranges.mdx"
---
...
Ranges of integers have the added convenience of being able to be used in a [FOR loop](../../statements/for.md).
...
The comparison form is faster, so prefer it on large tables. A pair of comparisons on an indexed field uses the index, while `IN` with a range reads every record in the table. See [numeric ranges in a `WHERE` clause](../../statements/select.md#numeric-ranges-in-a-where-clause) for details.
...
This opens up a range of functional programming patterns that are made possible by SurrealDB's [array functions](../../functions/database-functions/array.md), many of which can use [anonymous functions](closures.md) (closures) to perform an operation on each item in the array.
