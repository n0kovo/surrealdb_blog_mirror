...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/agent-memory/cookbooks/patterns/spoiler-safe-narrative-memory.mdx"
---
...
See [Temporal validity](https://surrealdb.com/docs/agent-memory/reasoning/temporal-validity#narrative-playback-and-spoiler-safety) for the model; this page is the recipe.
...
- **`asOf` on `/query`** walks known-time on attributes and relations; pair with [Temporal validity](https://surrealdb.com/docs/agent-memory/reasoning/temporal-validity) for `valid_from` / `valid_until` on in-world dates.
- Response caching is bypassed when temporal filters are present — correct for narrative playback, slightly higher latency.

## Related reading

- [Storing memories](https://surrealdb.com/docs/agent-memory/ingest/experiential/remember) — `observed_at` and `--as-of` on the CLI
- [Uploading documents](https://surrealdb.com/docs/agent-memory/ingest/authoritative/uploading-documents) — `observedAt` in upload metadata
- [Recalling memories](https://surrealdb.com/docs/agent-memory/retrieve/recall) — `asOf` and `occurredAt` on hits
- [Contexts and scope](https://surrealdb.com/docs/agent-memory/mental-model/contexts-and-scope) — `lens` and labels
