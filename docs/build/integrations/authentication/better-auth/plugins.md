...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/build/integrations/authentication/better-auth/plugins.mdx"
---
...
> [!IMPORTANT]
> Each plugin may add new tables. Re-run [schema generation](getting-started.md#schema-generation) after adding plugins to produce the updated DDL, or use `schemaMode: 'schemaless'` to accept the new fields without regenerating.

## SurrealDB helper functions

When you use the `organization` plugin (with or without the `teams` option), schema generation also emits a set of `fn::*` SurrealQL functions you can call directly in your own queries, [rules, and permissions](../../../../reference/query-language/statements/define/table.md#defining-permissions). These functions are defined with `IF NOT EXISTS`, so re-running the generated schema is safe.
