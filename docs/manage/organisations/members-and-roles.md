...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/manage/organisations/members-and-roles.mdx"
---
...
The invitation appears under **Pending** until the person accepts it. An invitee without an account creates one first, then joins the organisation on the role you chose. See [Accounts and sign-in](sign-in.md).
...
`surrealctl team invite` sends an invitation. `surrealctl team list` reports current membership, which helps when you review access across several organisations. See [surrealctl organisations](../surrealctl/organisations.md).
...
| Change instance configuration and [capabilities](../instances/configure.md#capabilities) | Yes | Yes | No |
| Invite and remove members | Yes | Yes | Own membership only |
| Change another member's role | Yes | No | No |
| View [invoices](billing.md) | Yes | Yes | No |
...
- **Delete an instance:** destroys the data in it. See [Configure an instance](../instances/configure.md#delete-an-instance).
- **Change [network access](../instances/network-access.md):** decides which external destinations a query can reach.
- **Restore a [backup](../instances/backups.md):** creates a new instance that holds a copy of production data.
...
- **[Organisations](index.md):** The organisation view and its settings.
- **[Accounts and sign-in](sign-in.md):** How an invitee creates an account.
- **[surrealctl organisations](../surrealctl/organisations.md):** Teams, invitations, and tokens from the command line.
