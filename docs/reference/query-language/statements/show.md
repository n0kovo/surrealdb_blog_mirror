...
* The `versionstamp` output above is due to an extra two bytes needed for more detailed ordering in the FoundationDB distributed [SurrealDB backend](../../../learn/data-models/architecture.md). To turn these versionstamps into a normal sequence of numbers, a right shift of sixteen bits (`>> 16`) can be used.
