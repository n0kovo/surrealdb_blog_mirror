...
| gRPC *Since v3.3.0* | The HTTP `/rpc` limit, so 4 MiB per message unless that is raised, in each direction | `SURREAL_GRPC_MAX_MESSAGE_SIZE` |
...
For request bodies on the other HTTP endpoints, see [Request size limits](http-protocol.md#request-size-limits).
...
- `SURREAL_GRPC_MAX_ATTACHED_SESSIONS` and `SURREAL_GRPC_NOTIFICATION_BUFFER` bound the sessions attached across all gRPC connections and the live query notifications buffered for a slow subscriber. All three variables are listed under [HTTP server config](../cli/surrealdb-cli/environment-variables.md#http-server-config).
- The service's `Health` method is the gRPC health check, and the `health` [HTTP route](../../learn/security/authorization/capabilities.md#http-route-names) allows or denies it together with `GET /health`. The server does not implement the standard `grpc.health.v1.Health` service, so a probe that calls that service, such as a Kubernetes `grpc` probe, fails with `Unimplemented`. Probe [`GET /health`](http-protocol.md#health) or [`GET /ready`](http-protocol.md#ready) over HTTP instead.
...
See [Errors](errors.md) for the full error shape, the list of kinds, and the wire codes.
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td colspan="2" scope="row" data-label="Description">
                The request `id` of the streaming query to stop, with the same value and type the `query_stream` request used
            </td>
        </tr>
    </tbody>
</table>
...
- `query_stream` is an [RPC method capability](../../learn/security/authorization/capabilities.md#rpc-method-names) of its own, and the query runs through the `query` method's gate as well, so denying either method denies streaming.

Both environment variables are listed under [WebSocket config](../cli/surrealdb-cli/environment-variables.md#websocket-config).
...
The result is the authentication token as a string. When a record access method is defined `WITH REFRESH`, the result is instead an object holding the token as `access` and a [refresh token](../query-language/statements/define/access/record.md#with-refresh-token) as `refresh`.
...
The result is the authentication token as a string. When the record access method is defined `WITH REFRESH`, the result is instead an object holding the token as `access` and a [refresh token](../query-language/statements/define/access/record.md#with-refresh-token) as `refresh`.
