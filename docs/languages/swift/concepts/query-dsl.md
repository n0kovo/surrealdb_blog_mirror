...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/swift/concepts/query-dsl.mdx"
---

# Query DSL

In addition to the high-level CRUD methods, the Swift SDK lets you build queries with a type-safe DSL. You can use either compile-time expression macros or the programmatic `SurrealDSL` builder, then run the result with [`query`](../methods/query.md).
...
Run a query with the [`query`](../methods/query.md) method:
...
For arbitrary SurrealQL that the DSL does not model, use [`queryRaw`](../methods/query-raw.md) with bound parameters.
