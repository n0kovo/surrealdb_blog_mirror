...
**`limit`** on `/query` and **`k`** on document query default to **10** and are capped at **50** (operator-tunable downward only — see [Configuration](configuration.md#request-and-list-limits)). The internal retrieval pool is fixed separately from `k`.
