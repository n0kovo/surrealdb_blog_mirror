...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/manage/schema-migration/testing/index.mdx"
---
...
> [!NOTE]
> `surrealkit test` applies a filesystem sync before running, so the [empty source set check](../sync.md#empty-source-sets) applies to it. A suite whose fixtures own the complete schema, with nothing in `database/schema`, needs `--no-sync`.
...
> [!IMPORTANT]
> Assertions that go red after upgrading from 0.7 were most likely never being evaluated, because a missing path used to pass. Check the shape of the actual result before changing the assertion. See [Upgrading](../upgrading.md#if-you-use-the-tester).
...
- [CI / CD](ci-cd.md): integrate tests into automated pipelines with GitHub Actions and Docker Compose
