...
For Prometheus pull behaviour, naming rules, the public `PUBLIC_METRICS` allowlist, and multi-tenant guidance common to both editions, see [Observability (metrics and Prometheus)](observability.md). For OTLP push, intervals, and legacy HTTP/RPC instruments, see [Telemetry (OTLP)](telemetry.md).
...
Redaction (`SURREAL_AUDIT_REDACT_*` / `SURREAL_SLOW_QUERY_REDACT_*`) runs on the executor thread **before** the queue so the file sink and OTel path see the same scrubbed text. Full knob lists live with the rest of the server [environment variables](../../reference/cli/surrealdb-cli/environment-variables.md).
...
Legacy series names were collapsed into labelled families in SurrealDB 3.1. The same migration table applies to Community and Enterprise; see [Observability (metrics and Prometheus)](observability.md) for the summary, [Telemetry (OTLP)](telemetry.md) for wire-format and deprecation notes, and the [Metrics reference](metrics.md#migration-from-30) for the full migration table.
...
- [Audit logging](audit-logging.md) — record shape, rotation, overflow semantics, hash chaining, redaction.
- [Slow-query logging](slow-query-logging.md) — triage pipeline mirroring the audit observer.
- [Configuration](configuration.md) — all `SURREAL_AUDIT_*` and `SURREAL_SLOW_QUERY_*` variables and the compliance checklist.
