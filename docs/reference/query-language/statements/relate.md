...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/query-language/statements/relate.mdx"
---
...
- Edge tables have two required fields `in` and `out`, which specify the directions of the relationships. These cannot be modified in schema declarations except to specify that they must be of a certain record type or to [add assertions](define/field.md#asserting-rules-on-fields).

Otherwise, edge tables behave like normal tables in terms of [updating](update.md), [defining a schema](define/table.md) or [indexes](define/indexes.md).

Another option for connecting data is using [record links](../language-primitives/record-links.md). Record links consist of a field with record IDs that serve as unidirectional links by default, or bidirectional links if reference tracking is used. The key differences are that graph relations have the following benefits over record links:
...
```
                     ...
                     │            │ │                            │                                                                              │ │                                          │ │ │                 │                                         │ │ │                                    │               
        ╭────────╮   │  ╭──────╮  │ │    ╭────╮     ╭────────╮   │   ┌──────────────┐     ╭────╮     ┌────────┐     ╭────╮     ┌────────────┐   │ │    ╭─────╮     ┌─────────────────────┐   │ │ │    ╭────────╮   │               ╭────────╮                │ │ │    ╭─────────╮     ┌───────────┐   │   ╭───╮       
├┼──────│ RELATE │───╯──│ ONLY │──╰─╯────│ OR │─────│ UPDATE │───╰───│ @from_record │─────│ -&gt; │─────│ @table │─────│ -&gt; │─────│ @to_record │───╯─╯────│ SET │─────│ @field = @value ... │───╰─╰─╯────│ RETURN │───┼───────────────│ BEFORE │────────────────┼─╰─╯────│ TIMEOUT │─────│ @duration │───╰───│ ; │─────┼┤
        ╰────────╯      ╰──────╯         ╰────╯     ╰────────╯       └──────────────┘     ╰────╯     └────────┘     ╰────╯     └────────────┘          ╰─────╯     └─────────────────────┘            ╰────────╯   │               ╰────────╯                │        ╰─────────╯     └───────────┘       ╰───╯       
                                                                                                                                                                                                                   │                                         │                                                        
                                                                                                                                                                                                                   │                ╭───────╮                │                                                        
                                                                                                                                                                                                                   ╰────────────────│ AFTER │────────────────╯                                                        
...
If you prefer a hard error at this point, [`INSERT RELATION`](insert.md#insert-relation-tables) can be used:
...
By default, the edge table gets created as a schemaless table when you execute the `RELATE` statement. You can make the table schemafull by [defining a schema](define/table.md).

A common use case is to make sure only unique relationships get created. You can do that by [defining an index](define/indexes.md).
...
To enforce unidirectional relationships, you can restrict the type definition using a [`DEFINE FIELD`](define/field.md) definition.
...
## Using [`LET`](let.md) parameters in RELATE statements

You can also use [parameters](../language-primitives/parameters.md) to specify the record IDs.
...
## Modifying output with the [`RETURN`](return.md) clause
...
You can also delete graph edges between two records in the database by using the [DELETE statement](delete.md).
...
For example, the `VALUE` clause inside a [`DEFINE FIELD`](define/field.md) statement is calculated every time a record is altered (that is, every time it is created or updated). If this value depends on a graph edge, creating the record first will cause `VALUE` to calculate it based on a nonexistent path.
...
[Destructuring](../language-primitives/idioms.md#destructuring) can also be used to pick and choose which fields to access inside a graph query. The following query will return the same output as above, except that `original_name: 'Πολιτεία'` will no longer show up.
...
Here we can use the [`array::complement`](../functions/database-functions/array.md#arraycomplement) function to return only items from one array that are not present in another array.
...
Adding a unique key is a good practice for this sort of relation, as it will prevent it from being created twice. This can be done by [defining a field](define/field.md) as a unique key based on the ordered record IDs involved, followed by a [`DEFINE INDEX`](define/field.md) statement.
...
The [destructuring syntax](../language-primitives/idioms.md#destructuring) can be used to reduce some typing. Here is the same query as the last using destructuring syntax instead of one line for each field.
...
The same condition can be written in square brackets - `->knows[WHERE has_dated]->person` - but that is the generic [array filter](../language-primitives/data-types/arrays.md#filtering-and-mapping-with-array-functions) applied after walking every edge. Both return the same records, and on a record with many edges only the parenthesised form stays fast.
...
Because the `WHERE` clause simply checks for [truthiness](../language-primitives/data-types/values.md#values-and-truthiness) (whether a value is present and not empty), these fields do not necessarily need to be booleans and can even be complex objects.
...
However, clauses available in [`SELECT` statements](select.md) such as `WHERE`, `LIMIT`, `GROUP BY`, aliases and so on can be used, making a graph clause the most flexible option.
