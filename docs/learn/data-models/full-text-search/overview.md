...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/data-models/full-text-search/overview.mdx"
---
...
Note: SurrealDB has many other built-in ways of working with text besides full-text search. For more details, see [Other ways to work with text](other-ways-to-work-with-text.md).
...
- **Powerful query language**: SurrealQL blends SQL-like syntax with searching syntax (the [`@@` matching operator](../../../reference/query-language/language-primitives/operators.md#matches-a-idmatchesa) for FTS queries, advanced indexing features, and so on).
...
- [Analyzers and tokenizers](analyzers-and-tokenizers.md): `DEFINE ANALYZER`, tokenizers, filters, stemming, and `search::analyze`.
- [Search indexes](search-indexes.md): `FULLTEXT ANALYZER` on a single field per index.
- [Scoring and ranking](scoring-and-ranking.md): the `@@` operator, BM25, highlights, and `search::score` / `search::highlight`.
- [Other ways to work with text](other-ways-to-work-with-text.md): sorting and collating text, contains, fuzzy matching, regex, and related string tools.

For reference, see [`DEFINE INDEX`](../../../reference/query-language/statements/define/indexes.md), [`DEFINE ANALYZER`](../../../reference/query-language/statements/define/analyzer.md), and [Search functions](../../../reference/query-language/functions/database-functions/search.md).
