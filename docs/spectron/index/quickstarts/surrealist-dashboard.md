...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/spectron/index/quickstarts/surrealist-dashboard.mdx"
---

# Surrealist dashboard quickstart

Spectron **Contexts** live inside **[Surrealist](https://surrealdb.com/surrealist)** as a SurrealDB Cloud product — alongside database instances, not a separate console. This page is a UI-first tour: **Playground**, **Memory**, **Documents**, and **API keys** — before you reach for an SDK or the [REST API](../../reference/rest-api.md).
...
You do not need the CLI for day-to-day exploration — **Playground**, **Memory**, **Documents**, and **Scopes** call the same Spectron data API your application uses. For scripted **remember** / **recall**, use [Hosted quickstart](https://surrealdb.com/docs/spectron/quickstarts/hosted) or an [SDK](../../integrations/index.md).
...
- **Access is deny-by-default.** A principal sees a scope node's facts only when explicitly **granted** that path (or a pattern that covers it). Grants do **not** implicitly flow up or down the folder tree — see [Scope as security boundary](../../self-hosting/security/scope-as-security-boundary.md).
- **Tombstone a scope node** (`scope:delete` in the API) marks the folder as retired in the vocabulary — **reversible** in principle. **`POST /scopes/forget`** is different: it **permanently erases** all facts tagged under that subtree. See [Forgetting memories](../../agent-memory/operations/forget.md#scoped-forget--erase-a-whole-subtree).

Registering a path does **not** empty or pre-create a memory bucket. Chat and uploads can land in your **write region** before you register anything in **Scopes** — the tree is navigation and governance vocabulary as much as a prerequisite. See [Default write region](https://surrealdb.com/docs/spectron/mental-model/contexts-and-scope#default-write-region).
...
Supported uploads include plain text, markdown, JSON, HTML, PDF, and (depending on your context's ingestion profile) images, audio, and video. See [Uploading documents](../../agent-memory/ingest/authoritative/uploading-documents.md) for MIME types and pipeline behaviour.
...
See [API keys and delegation](../../self-hosting/security/api-keys-and-delegation.md#self-service-keys-and-cloud-brokerage) for broker semantics on the Spectron side.
...
Use **`Authorization: Bearer sk-ctx-…`** on the data plane — see [REST API](../../reference/rest-api.md). The **Integrations** view also includes copy-ready snippets (cURL, JavaScript, Python, Rust, and others).
...
Playground and other dashboard writes tag memory with your principal's **write region** when you have not chosen a narrower path. Facts can exist before you register paths in the **Scopes** tab — that view is the scope **vocabulary** (browsing and registering slash paths), not a prerequisite empty bucket. Register paths when you want explicit partitioning or org/user hierarchy; see [Contexts and scope](https://surrealdb.com/docs/spectron/mental-model/contexts-and-scope#default-write-region).
...
- **[Hosted quickstart](https://surrealdb.com/docs/spectron/quickstarts/hosted)** — first **remember** / **recall** from your terminal
- **[Uploading documents](../../agent-memory/ingest/authoritative/uploading-documents.md)** — REST upload, polling, and scopes
- **[Integrations](../../integrations/index.md)** — Python, TypeScript, and Swift SDKs
- **[Contexts and scope](https://surrealdb.com/docs/spectron/mental-model/contexts-and-scope)** — slash-path scope model
- **[Eight pillars and six categories](https://surrealdb.com/docs/spectron/architecture/eight-pillars-and-categories)** — how Playground and Documents map onto Spectron's memory model
