...
Some workflows rely on creating the edge first (for example, a street with predictable house IDs before houses exist. A [`DEFINE FIELD ... VALUE`](../../../reference/query-language/statements/define/field.md) can describe the path from `house` to `street` and resolve once the house exists:
