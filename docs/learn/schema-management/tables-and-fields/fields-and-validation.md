...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/schema-management/tables-and-fields/fields-and-validation.mdx"
---
...
Computed fields (derived on every read) have their own learn page: [Computed fields](../computed-data/computed-fields.md).
...
A `|` vertical bar can be used to allow a field to be one of a set of types. The following example shows a field that can be a [`UUID`](../../../reference/query-language/language-primitives/data-types/uuids.md) or an [`int`](../../../reference/query-language/language-primitives/data-types/numbers.md#integer-numbers), perhaps for `user` records that have varying data due to two diffent legacy ID types.
...
The example below shows how to define a field `user` on a `POST` table. The field is of type [record](../../../reference/query-language/language-primitives/record-links.md). This means that the field can store a `record<user>` or `NONE`.
...
In the example below, the `VALUE` clause is used to ensure that the email address is always stored in lowercase characters by using the [`string::lowercase`](../../../reference/query-language/functions/database-functions/string.md#stringlowercase) function.
...
As the `ASSERT` clause expects an expression that returns a boolean, an assertion with a custom message can be manually created by returning `true` in one case and using a [`THROW`](../../../reference/query-language/statements/throw.md) clause otherwise.
...
By default, the permissions on a field are set to `FULL` unless otherwise specified. That means once a [table](tables.md#defining-permissions) allows an operation for a record user, the field is included unless you narrow it with your own `PERMISSIONS` clause. Tables default the other way: omitting table `PERMISSIONS` stores `PERMISSIONS NONE`.
...
Like table permissions, field permissions apply to [record users](../../security/authentication/authentication.md#record-users) (and guests when enabled), not to system users.
