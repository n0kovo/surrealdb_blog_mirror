...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/cli/surrealctl/global-flags.mdx"
---
...
`--token` has no short form on purpose: `-t` reads as `--type` on the instance commands, and a credential belongs in the environment or a file rather than in `argv`. See [Authentication](authentication.md#personal-access-tokens).
...
4. The persisted context, written by [`org use`](commands/org.md#org-use)
...
`--debug` prints which layer won each value, and [`context show`](commands/context.md#context-show) reports it as part of its answer:
...
The only `SURREAL_*` variables involved are ones `surrealctl` *sets* for the `surreal` child process during [the handoff](../../../manage/surrealctl/install.md#the-surreal-handoff): `SURREAL_TOKEN`, `SURREAL_NAMESPACE` and `SURREAL_DATABASE`.
...
| `EDITOR` | Used by [`config edit`](commands/config.md#config-edit) |

## Configuration file equivalents

Five values can be persisted per profile with [`config set`](commands/config.md#config-set). Each is outranked by its environment variable, and `config set` warns when that variable is exported.
