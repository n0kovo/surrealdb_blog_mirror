...
To make the output easier to read, we'll do the following. First we'll put the two queries creating the records inside their own scope, and use the [value::chain()](../../../reference/query-language/functions/database-functions/value.md#chain) function to grab the output, ignore it, and turn it into a string showing what operation has just been performed. Then we'll do the same for a regular `SELECT` query over the `weather2` data, compared to the record range query for the `weather` data. If you need the query to stop when a step does not look right, use [`.expect()`](../../../reference/query-language/functions/database-functions/value.md#valueexpect) on that value instead of `.chain()` to return the original value when your closure returns `true`, otherwise failing with an error.
...
Here is one example of an event that creates an `alert` whenever a weather condition is set to critical, and also uses an [http function](../../../reference/query-language/functions/database-functions/http.md) to let an external service know about it.
