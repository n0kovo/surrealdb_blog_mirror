...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/cli/surrealctl/commands/misc.mdx"
---
...
`whoami` spends a request on the API's own profile route, which makes it the counterpart to [`auth status`](auth.md#auth-status): that one reports what is on this machine, without touching the network; this one reports what the API believes. When they disagree, the API is right.
...
This command has no options of its own — only the [global flags](../global-flags.md).
...
Accepts the [list presentation flags](../output-and-exit-codes.md#list-presentation-flags). Column ids are `check`, `ok` and `detail`.
...
The `surreal binary` check reports which copy is in use and does not offer to download one; a diagnostic should not fetch a binary. See [the `surreal` handoff](../../../../manage/surrealctl/install.md#the-surreal-handoff).
...
See [Installation](../../../../manage/surrealctl/install.md#shell-completion) for where each shell expects the file to live.

## Related pages

- [Output and exit codes](../output-and-exit-codes.md) — the `--json` contract and the full exit-code table
- [Authentication](../authentication.md) — what `whoami` and `status` are reporting on
- [Installation](../../../../manage/surrealctl/install.md) — completion setup and the `surreal` handoff
- [Overview](../overview.md) — the rest of the reference
- [SurrealDB CLI](../../surrealdb-cli/overview.md) — for queries, imports, exports and running a server
