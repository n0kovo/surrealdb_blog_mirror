...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/data-models/vector-search/vector-indexes.mdx"
---

# Vector indexes

The simplest way to search through your vector embeddings and data in SurrealDB is the [brute force approach](../../../reference/query-language/statements/define/indexes.md#brute-force-method).
...
| --- | --- | --- |
| [HNSW](https://en.wikipedia.org/wiki/Hierarchical_navigable_small_world) ([`DEFINE INDEX … HNSW`](../../../reference/query-language/statements/define/indexes.md#hnsw-hierarchical-navigable-small-world)) | Low-latency ANN when the graph fits comfortably in memory with headroom for the bounded vector cache | In-memory hot graph + persistence |
| [DISKANN](https://arxiv.org/abs/1907.01668) ([`DEFINE INDEX … DISKANN`](../../../reference/query-language/statements/define/indexes.md#diskann-disk-based-approximate-nearest-neighbours)) *(SurrealDB 3.1+)* | Very large corpora where RAM cannot hold the full graph - full-precision vectors and graph stay in the key-value store and are paged through a bounded cache | Key-value-backed graph + bounded in-memory cache (not on **WASM** targets) |

Both are [proximity graph](../../../reference/query-language/statements/define/indexes.md#hnsw-hierarchical-navigable-small-world)-style indexes. Queries use the same [`<|K, …|>` KNN operator shapes](../../../reference/query-language/language-primitives/operators.md#knn); the optimiser picks the index when distances and types line up.
...
For more details, see the [`DEFINE INDEX` statement](../../../reference/query-language/statements/define/indexes.md#hnsw-hierarchical-navigable-small-world) documentation.
...
With a DISKANN index defined on `point`, the same `<|K, EF|>` approximate form applies; the second number bounds the dynamic candidate list for search (see the [KNN operator](../../../reference/query-language/language-primitives/operators.md#knn)).
...
- Verify index utilisation in queries using the [`EXPLAIN FULL` clause](../../../reference/query-language/statements/select.md#the-explain-clause). E.g: `SELECT id FROM pts WHERE point <|10|> [2,3,4,5] EXPLAIN FULL;`
