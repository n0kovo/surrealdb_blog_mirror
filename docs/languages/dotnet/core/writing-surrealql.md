...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/index/languages/dotnet/core/writing-surrealql.mdx"
---

# Run SurrealQL queries

The methods below are used to interact with the database and perform CRUD operations. You can also use the `query` method to run [SurrealQL statements](../../../reference/query-language/statements/overview.md) against the database.
...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Method"><a href="#query"> ` db.Query() `</a></td>
			<td scope="row" data-label="Description">Runs a set of SurrealQL statements against the database</td>
		</tr>
        <tr>
            <td scope="row" data-label="Method"><a href="#raw_query"> ` db.RawQuery() `</a></td>
