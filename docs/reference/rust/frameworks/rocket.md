...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/rust/frameworks/rocket.mdx"
---
...
* Two endpoints to allow signing up and signing in as a [record user](../../query-language/statements/define/access/record.md).
...
With the database running, we will now connect to the database "main" located in the namespace "main". You can connect to it by [creating a connection](../../../explore/studio/index.md) inside SurrealDB Studio, or by using the following command to start an interactive shell.
...
Next, we will put the database client together. Rocket provides a [`.manage()`](https://rocket.rs/guide/v0.5/state/) method when starting a router that would give us access to the database inside its functions. However, for simplicity we can instead wrap the client inside a `LazyLock` to make it into a global static. Either approach works here, though note that cloning a `Surreal<C>` to hand to `.manage()` opens a [new session](../concepts/multi-tenancy.md) rather than making a cheap copy of the handle, so wrap the client in an `Arc` if the same client is cloned elsewhere in the application.
...
For an actual user in production, you would probably want to require an email and some other fields. Functions like [`string::is_email`](../../query-language/functions/database-functions/string.md#stringis_email) can be used to ensure that the value passed in is valid.

```surql
DEFINE FIELD email ON TABLE user TYPE string ASSERT $value.is_email();
```

However, for this simple example, each user will simply have a unique name and a password. The password will be stored in hashed and salted form on the database, making it unique and unreadable every time it is generated. The only way to check if it is correct is by using a [compare function](../../query-language/functions/database-functions/crypto.md#cryptoargon2compare) of the output with an attempted password. Here is a short SurrealQL sample to show how the process works.
...
* Using the [`AUTHENTICATE`](../../query-language/statements/define/access/record.md#example-failing-authentication) clause inside the `DEFINE ACCESS` statement. This will result in increased performance thanks to only being executed once, compared to permissions checks which are executed for each query.
* Adding some interesting behaviour to the database such as [changefeeds](../../query-language/statements/define/table.md#example-usage) or [events](../../query-language/statements/define/event.md).
