...
Two-factor authentication configured with Google or GitHub also applies to your SurrealDB account, because the provider verifies you before returning you to SurrealDB. For an account that signs in with an email address and password, add an authenticator app instead. See [Two-step verification](security.md) for more.
...
Each person needs their own account. Invite colleagues to the organisation instead of sharing credentials. Separate accounts let each member hold a different role, and let you remove someone without changing a password the rest of the team uses. See [Members and roles](members-and-roles.md).

For automation, use a token from [`surrealctl`](../surrealctl/authentication.md) rather than an interactive account.

## Next steps

1. [Create an organisation](index.md) to hold your instances and billing.
2. [Deploy an instance](../instances/create.md).
3. [Connect to it](../instances/connect/index.md) from SurrealDB Studio, the CLI, an SDK, or over HTTP.
