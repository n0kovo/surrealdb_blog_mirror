...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/javascript/api/queries/query.mdx"
---
...
<table>
    ...
    <tbody>
        <tr>
            <td>`queryIndexes` <label label="optional" /></td>
            <td>`number[]`</td>
...
#### Returns
`Promise<Responses<T, J>>` - Array of [`QueryResponse`](../types/index.md#queryresponse) objects
...
By default, a conflict is detected using [`isRetryableConflict`](../utilities/is-retryable-conflict.md), which matches on the server's error message. You can supply a custom predicate to override this:
