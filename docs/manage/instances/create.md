...
- An organisation to hold the instance. See [Organisations](../organisations/index.md).
...
The plan fixes the topology of the instance. To move a workload from Start to Scale, deploy a Scale instance, then [restore a backup](backups.md) or [import an export](import-and-export.md) into it. Choose **Scale** if the workload must survive the loss of a node. See [High availability](high-availability.md).
...
| **Restore from backup** | A copy of an existing instance, taken from one of its [backup snapshots](backups.md) |
...
2. Enter payment details if the organisation does not have them yet. See [Billing](../organisations/billing.md).
3. Confirm the order.
...
3. Open the instance and check that the [dashboard](monitoring.md) reports the version, region, and instance type you chose.
4. [Connect to the instance](connect/index.md) and run a query.

If the instance started empty, it has no namespace or database yet. Create both before you run a query. See [Connect via SurrealDB Studio](connect/via-studio.md).

## Deploying from the command line

`surrealctl instance create` performs the same deploy, which is what you want in CI or in a provisioning script. See [surrealctl instances](../surrealctl/instances.md).

## Next steps

- **[Connect to an instance](connect/index.md):** SurrealDB Studio, the CLI, SDKs, and HTTP.
- **[Configure an instance](configure.md):** capabilities, compute, and storage.
- **[Monitoring](monitoring.md):** confirm the instance is healthy.
