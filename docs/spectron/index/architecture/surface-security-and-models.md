...
- **Prompt-injection handling** – ingest-time scanning on uploads and batched turns; untrusted text is sanitised before it is interpolated into extraction, chat, and consolidation prompts (see [Extraction pipeline](../../agent-memory/reasoning/extraction-pipeline.md#untrusted-metadata-in-prompts)).
