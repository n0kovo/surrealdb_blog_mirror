...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/rest-api/http-protocol.mdx"
---
...
The [`surreal start`](../cli/surrealdb-cli/commands/start.md) command without any arguments is all that is needed to start a server at the default `http://localhost:8000` address. Many examples below assume the flags `--user root` and `--pass secret` to create a root user with the name `root` and password `secret`. The `--unauthenticated` flag can be used when experimenting to turn off authentication, effectively allowing root access by any and all connections.

The [local database serving](../../explore/studio/index.md) functionality on the SurrealDB Studio can also be used to start a server.
...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td scope="row" data-label="Function"><a href="#ml-import">`POST /ml/import`</a></td>
            <td scope="row" data-label="Description">Import a SurrealML model into a specific Namespace and Database</td>
        </tr>
        <tr>
            <td scope="row" data-label="Function"><a href="#ml-export">`GET /ml/export/:name/:version`</a></td>
            <td scope="row" data-label="Description">Export a SurrealML model from a specific Namespace and Database</td>
        </tr>
        <tr>
            <td scope="row" data-label="Function"><a href="#custom">`/api/:namespace/:database/:endpoint`</a></td>
...
| `POST /mcp` *Since v3.1.0* | 4 MiB | `SURREAL_HTTP_MAX_MCP_BODY_SIZE` |

The WebSocket `/rpc` endpoint is not bound by an HTTP body size. It allows up to 128 MiB per message (`SURREAL_WEBSOCKET_MAX_MESSAGE_SIZE`). As most SDKs use the WebSocket engine by default, they are able to send payloads that the plain HTTP endpoints reject. See [Message size limits](rpc-protocol.md#message-size-limits) for how the server ceiling interacts with the limit an SDK applies on its own side.
...
* For ordinary queries, connect through a client SDK such as [Rust](../rust/index.md) or [JavaScript](../javascript/index.md). The WebSocket engine raises the per-message ceiling to 128 MiB.
* To stay on plain HTTP, use [`POST /rpc`](rpc-protocol.md) for 4 MiB.
* For bulk data loading, use [`POST /import`](#import), which accepts up to 4 GiB per request.

### Tuning the limits

Every limit above is set by an [environment variable](../cli/surrealdb-cli/environment-variables.md#http-server-config) on a self-hosted server. From 3.0 those variables accept byte-size suffixes such as `16MiB` or `4GB`; on 2.x they take a raw byte count.
...
When the server is started with [`--query-timeout`](../cli/surrealdb-cli/environment-variables.md#command-environment-variables) (`SURREAL_QUERY_TIMEOUT`), the same duration also limits the wall-clock time of a whole request on these endpoints:

- [`POST /sql`](#sql) and the `/sql` WebSocket.
- [`POST /gql`](#gql).
- [`POST /graphql`](#graphql), for a single request and for a batch.
- `POST /rpc`, for every [RPC method](rpc-protocol.md), in the same way as on the WebSocket `/rpc` endpoint and over gRPC. This includes `signin`, `signup` and `authenticate`.
...
This HTTP RESTful endpoint checks whether the storage engine responds, by reading one key in a read-only transaction. It returns `200` when the read succeeds and `500` when it fails, or `403` when the `health` route is denied by [capabilities](../../learn/security/authorization/capabilities.md#http-route-names).
...
The CLI [`surreal isready`](../cli/surrealdb-cli/commands/isready.md) command calls this endpoint.
...
The body is streamed: the server parses and applies statements as the bytes arrive rather than buffering the whole file. This is the endpoint to use for bulk data loading, since it accepts far more than [`/sql`](#sql) or [`/rpc`](rpc-protocol.md) do.
...
When [authentication rate limiting](../cli/surrealdb-cli/environment-variables.md#authentication-rate-limiting-environment-variables) is enabled, an attempt beyond the client address's budget is refused with `429 Too Many Requests` and a `Retry-After` header giving the number of seconds to wait, and the error body says that too many authentication attempts were made from this client address. `/signin` and [`/signup`](#signup) draw on the same budget. *Since v3.3.0*
...
> [!IMPORTANT]
> The `ac` parameter is only required if you are signing in using an [access method](../query-language/statements/define/access/index.md) as a record user. For system users on the database, namespace, and root level, this parameter can be omitted.
...
When [authentication rate limiting](../cli/surrealdb-cli/environment-variables.md#authentication-rate-limiting-environment-variables) is enabled, an attempt beyond the client address's budget is refused with `429 Too Many Requests` and a `Retry-After` header giving the number of seconds to wait. `/signup` shares its budget with [`/signin`](#signin). *Since v3.3.0*
...
> [!NOTE]
> This endpoint is **not** [GraphQL](../../learn/querying/graphql/overview.md). GraphQL queries belong on [`POST /graphql`](#graphql).
...
Parse errors return **HTTP 400** with an error payload. See [GQL via HTTP](../../learn/querying/gql/via-http.md) for enabling GQL, seeding data, and RPC examples.
