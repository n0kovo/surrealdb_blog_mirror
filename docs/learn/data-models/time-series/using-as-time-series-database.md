...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/data-models/time-series/using-as-time-series-database.mdx"
---
...
Storing the same information as a record or as part of the ID might look similar at first glance, but internally it’s optimised for efficient range selection through [range queries](../../../reference/query-language/language-primitives/data-types/record-ids.md#record-ranges) on record IDs. As all values in SurrealDB can be [compared and sorted](../../../reference/query-language/language-primitives/data-types/values.md#comparing-and-ordering-values), putting the most important information into the record ID itself allows you to query only a range of record IDs instead of a whole table.
...
### IDs inside IDs
The complex record IDs above also contain record IDs of their own. This is known as a [record link](../../../reference/query-language/language-primitives/record-links.md), which allows us to reduce the fields in our main time series table to only the necessary ones by offloading most of the metadata to connected tables.

In our case we have:

- `location:Longleat_House`, which refers to the `Longleat_House` ID in the `location` table. There we put all the metadata about the location itself such as [geo coordinates](../../../reference/query-language/language-primitives/data-types/geometries.md).
...
For real-time visualisation of our metrics we can then use [Live Queries](../../../reference/query-language/statements/live-select.md) to stream real-time updates to our client, such as a BI dashboard or embedded analytics code.
...
#### Custom events
For something more bespoke, you can [create your own event triggers](../../../reference/query-language/statements/define/event.md) based on when a record is created, updated or deleted. You can include any valid SurrealQL inside the event.
...
5. Send an [http::post](../../../reference/query-language/functions/database-functions/http.md#httppost) request with the outlier details.
