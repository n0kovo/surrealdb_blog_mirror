...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/query-language/language-primitives/data-types/strings.mdx"
---
...
The `r` prefix tells the parser that the contents of the string represent a [`record ID`](record-ids.md). The parser expects record IDs to have the following format: `table_name:record ID`.
...
In the example below, using the [`type::is_string()`](../../functions/database-functions/type.md#typeis_string) and [`type::is_record()`](../../functions/database-functions/type.md#typeis_record) functions respectively, you can check the type of the string.
...
The `d` prefix tells the parser that the contents of the string represent a [`datetime`](datetimes.md). The parser expects `datetime` values to have a valid [RFC 3339](https://datatracker.ietf.org/doc/html/rfc3339) format. Here are a few examples:
...
![A screenshot showing how a string prefix allows incorrect UUID input to be identified before a query can be run. In this case, the parser is able to inform the user that an underscore at column 18 is the issue.](../../../../assets/img/image/light/surrealql-parse-error.png)
