...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/reference/query-language/clauses/with.mdx"
---
...
> [!NOTE]
> If you are using a `SELECT` statement, the `WITH` clause is used to specify the index to use for the query. You can define an index using the [`DEFINE INDEX`](../statements/define/indexes.md) statement. Also see the [`DEFINE ANALYZER`](../statements/define/analyzer.md) statement for more information on optimizing query performance with full-text search.
