...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/learn/querying/custom-apis/managing-apis.mdx"
---
...
That pattern pairs well with tightening access such as varying querying timeouts for free or entry-level users on an app. To accomplish this, you can combine [`DEFINE API`](../../../reference/query-language/statements/define/api.md) with [capabilities](../../security/authorization/capabilities.md) or server [environment variables](../../../reference/cli/surrealdb-cli/environment-variables.md) so certain users only see the APIs you designed.
...
Inside the handler you can use the built-in [`$request`](../../../reference/query-language/language-primitives/parameters.md#request) value. This parameter holds the `method`, `body`, `headers`, `query`, `params` (from your path pattern), and `context` you or middleware may have set.

## A minimal endpoint

Here is a small endpoint that echoes part of the body and sets a couple of headers. The precise clauses and permissions are spelled out in the [`DEFINE API`](../../../reference/query-language/statements/define/api.md) reference page.
...
You can test the endpoint from SurrealQL without needing to deploy by calling the [`api::invoke`](../../../reference/query-language/functions/database-functions/api.md) function. This function takes either the path alone or the path plus a body object.
...
Built-in helpers such as `api::timeout` sit in the `MIDDLEWARE` list before your `THEN` block. For more details on how to use middleware, see the [next page](middleware.md).
