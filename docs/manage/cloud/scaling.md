...
Move to **Scale** when a single node is no longer enough — because you need **fault tolerance**, sustained query throughput across nodes, or both — not only because disk is full. Scale runs a minimum of **three compute units**; see [High availability](high-availability.md) for why that matters.
...
![Vertical scaling: a single SurrealDB compute node paired with dedicated storage — resizing means giving this node more CPU, memory, and I/O.](../../assets/img/image/cloud/light/start-single-node-light.png)
...
![Horizontal scaling: multiple SurrealDB compute nodes backed by SurrealDS distributed storage.](../../assets/img/image/cloud/light/horizontal-scaling-light.png)

See [Cloud architecture](../../build/deployment/surrealdb-cloud/architecture.md) for how Start and Scale differ.
...
For operational scaling detail in the deployment guide, see [Scaling operations](../../build/deployment/surrealdb-cloud/operations/scaling.md).
