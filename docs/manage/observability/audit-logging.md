...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/manage/observability/audit-logging.mdx"
---
...
The full set of configuration variables lives on the [configuration reference](configuration.md#audit-log-knobs).
...
<table>
    ...
    <tbody>
        <tr>
            <td scope="row" data-label="Event">`surrealdb.audit.statement`</td>
            <td scope="row" data-label="Severities">`Info` / `Error`</td>
            <td scope="row" data-label="Captures">Completion of an individual statement.</td>
        </tr>
        <tr>
            <td scope="row" data-label="Event">`surrealdb.audit.query`</td>
            <td scope="row" data-label="Severities">`Info` / `Error`</td>
            <td scope="row" data-label="Captures">Completion of a multi-statement query.</td>
        </tr>
        <tr>
            <td scope="row" data-label="Event">`surrealdb.audit.transaction`</td>
            <td scope="row" data-label="Severities">`Info` / `Error`</td>
            <td scope="row" data-label="Captures">Commit or rollback of a transaction.</td>
        </tr>
        <tr>
            <td scope="row" data-label="Event">`surrealdb.audit.rpc`</td>
            <td scope="row" data-label="Severities">`Info` / `Error`</td>
            <td scope="row" data-label="Captures">Completion of an RPC call.</td>
        </tr>
        <tr>
            <td scope="row" data-label="Event">`surrealdb.audit.auth`</td>
            <td scope="row" data-label="Severities">`Info` (success) / `Warn` (any non-success) / `Error` (`outcome=error`)</td>
            <td scope="row" data-label="Captures">Sign-in, sign-up and authentication outcomes.</td>
        </tr>
        <tr>
            <td scope="row" data-label="Event">`surrealdb.audit.session`</td>
            <td scope="row" data-label="Severities">`Info`</td>
            <td scope="row" data-label="Captures">Session connect and disconnect events.</td>
        </tr>
        <tr>
            <td scope="row" data-label="Event">`surrealdb.audit.http`</td>
            <td scope="row" data-label="Severities">`Info` / `Error` (typically 5xx responses)</td>
...
The slow-query pipeline exposes the same shape under the `surrealdb_slow_query_*` prefix — see [slow-query logging](slow-query-logging.md#pipeline-self-metrics).

## Related references

- [Configuration → Audit log knobs](configuration.md#audit-log-knobs) — every audit-log environment variable.
- [Configuration → Compliance checklist](configuration.md#compliance-checklist) — the minimum tamper-evident configuration.
- [Slow-query logging](slow-query-logging.md) — the sister pipeline for triage data.
- [Metrics reference → Audit log pipeline self-metrics](metrics.md#audit-log-pipeline-self-metrics) — the five gauges in the metric catalogue.
