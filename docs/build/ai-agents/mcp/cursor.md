...
source: "https://github.com/surrealdb/docs.surrealdb.com/blob/main/src/content/build/ai-agents/mcp/cursor.mdx"
---

# MCP in Cursor

With the [SurrealDB MCP Server](index.md) connected, Cursor's agent can reach your SurrealDB Cloud account while you work: check what is deployed, query a database, and read logs without leaving the editor.
...
Give the token only the permissions you want the agent to have. [Signing in](index.md#signing-in) lists what each one allows.
...
Cursor picks the instance, sets the namespace and database, and runs the SurrealQL, showing you each call as it goes. [Example usages](examples.md) has more, including deploying an instance and investigating a slow query.
